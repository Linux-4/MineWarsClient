...
public class PathNavigateGround extends PathNavigate {
	...
	public Path getPathToPos(BlockPos pos) {
		if (this.worldObj.getBlockState(pos).getMaterial() == Material.AIR) {
			...
			for (blockpos = pos.down(); blockpos.getY() > 0
					&& this.worldObj.getBlockState(blockpos).getMaterial() == Material.AIR; blockpos = blockpos
							.down()) {
				;
			}
			...
			while (blockpos.getY() < this.worldObj.getHeight()
					&& this.worldObj.getBlockState(blockpos).getMaterial() == Material.AIR) {
		...
		if (!this.worldObj.getBlockState(pos).getMaterial().isSolid()) {
			...
			for (blockpos1 = pos.up(); blockpos1.getY() < this.worldObj.getHeight()
					&& this.worldObj.getBlockState(blockpos1).getMaterial().isSolid(); blockpos1 = blockpos1.up()) {
				;
			}
	...
	private int getPathablePosY() {
		if (this.theEntity.isInWater() && this.getCanSwim()) {
			...
			while (block == Blocks.FLOWING_WATER || block == Blocks.WATER) {
				++i;
				block = this.worldObj.getBlockState(
						new BlockPos(MathHelper.floor(this.theEntity.posX), i, MathHelper.floor(this.theEntity.posZ)))
						.getBlock();
				++j;
	...
	protected void removeSunnyPath() {
		...
		if (this.shouldAvoidSun) {
			if (this.worldObj.canSeeSky(new BlockPos(MathHelper.floor(this.theEntity.posX),
			...
			for (int k = i; k < i + sizeX; ++k) {
				for (int l = j; l < j + sizeZ; ++l) {
					...
					if (d0 * p_179683_8_ + d1 * p_179683_10_ >= 0.0D) {
						PathNodeType pathnodetype = this.nodeProcessor.getPathNodeType(this.worldObj, k, y - 1, l,
								this.theEntity, sizeX, sizeY, sizeZ, true, true);
						...
						pathnodetype = this.nodeProcessor.getPathNodeType(this.worldObj, k, y, l, this.theEntity, sizeX,
