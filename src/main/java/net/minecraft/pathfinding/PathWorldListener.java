			...
			for (int j = this.navigations.size(); i < j; ++i) {
				...
				if (pathnavigate != null && !pathnavigate.canUpdatePathOnTimeout()) {
					...
					if (path != null && !path.isFinished() && path.getCurrentPathLength() != 0) {
						...
						double d0 = pos.distanceSq(((double) pathpoint.xCoord + pathnavigate.theEntity.posX) / 2.0D,
								((double) pathpoint.yCoord + pathnavigate.theEntity.posY) / 2.0D,
								((double) pathpoint.zCoord + pathnavigate.theEntity.posZ) / 2.0D);
