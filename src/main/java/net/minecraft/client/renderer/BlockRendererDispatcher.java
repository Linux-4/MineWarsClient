...
public class BlockRendererDispatcher implements IResourceManagerReloadListener {
	...
	public void renderBlockDamage(IBlockState state, BlockPos pos, TextureAtlasSprite texture,
		...
		if (state.getRenderType() == EnumBlockRenderType.MODEL) {
			...
			IBakedModel ibakedmodel1 = (new SimpleBakedModel.Builder(state, ibakedmodel, texture, pos))
					.makeBakedModel();
	...
	public boolean renderBlock(IBlockState state, BlockPos pos, IBlockAccess blockAccess,
		...
		try {
			...
			if (enumblockrendertype == EnumBlockRenderType.INVISIBLE) {
				...
				case MODEL:
					return this.blockModelRenderer.renderModel(blockAccess, this.getModelForState(state), state, pos,
