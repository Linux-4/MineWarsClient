		...
		try {
			...
			if (flag1 || flag2 || aboolean[0] || aboolean[1] || aboolean[2] || aboolean[3]) {
				...
				if (flag1) {
					...
					TextureAtlasSprite textureatlassprite = f12 > -999.0F ? atextureatlassprite[1]
							: atextureatlassprite[0];
				...
				if (flag2) {
					...
					int i2 = l1 >> 16 & 65535;
					int j2 = l1 & 65535;
					worldRendererIn.pos(d0, d1, d2 + 1.0D).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F)
							.tex((double) f38, (double) f41).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0, d1, d2).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F)
							.tex((double) f38, (double) f40).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0 + 1.0D, d1, d2).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F)
							.tex((double) f39, (double) f40).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0 + 1.0D, d1, d2 + 1.0D).color(f * 0.5F, f1 * 0.5F, f2 * 0.5F, 1.0F)
							.tex((double) f39, (double) f41).lightmap(i2, j2).endVertex();
					flag3 = true;
				}
				...
				for (int i1 = 0; i1 < 4; ++i1) {
					...
					if (!flag) {
						...
						if (block == Blocks.GLASS || block == Blocks.STAINED_GLASS || block == Blocks.BEACON
						...
						if (block instanceof BlockSlab) {
							...
							if (!blockslab.isDouble()
									&& iblockstate.getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.BOTTOM) {
								f42 = 0.5F;
								f43 = 0.5F;
							}
						}
					...
					if (aboolean[i1]) {
						...
						if (f44 > f42 || f45 > f43) {
							...
							worldRendererIn.pos(d3, d1 + (double) f44, d4).color(f35, f36, f37, 1.0F)
									.tex((double) f27, (double) f29).lightmap(k, l).endVertex();
							worldRendererIn.pos(d5, d1 + (double) f45, d6).color(f35, f36, f37, 1.0F)
									.tex((double) f28, (double) f30).lightmap(k, l).endVertex();
							worldRendererIn.pos(d5, d1 + (double) f43, d6).color(f35, f36, f37, 1.0F)
									.tex((double) f28, (double) f33).lightmap(k, l).endVertex();
							worldRendererIn.pos(d3, d1 + (double) f42, d4).color(f35, f36, f37, 1.0F)
									.tex((double) f27, (double) f32).lightmap(k, l).endVertex();
