...
public class DebugRendererSolidFace implements DebugRenderer.IDebugRenderer {
	...
	public void render(float p_190060_1_, long p_190060_2_) {
		...
		Iterable<BlockPos> iterable = BlockPos.func_191532_a(MathHelper.floor(entityplayer.posX - 6.0D),
				MathHelper.floor(entityplayer.posY - 6.0D), MathHelper.floor(entityplayer.posZ - 6.0D),
				MathHelper.floor(entityplayer.posX + 6.0D), MathHelper.floor(entityplayer.posY + 6.0D),
		...
		for (BlockPos blockpos : iterable) {
			...
			if (iblockstate.getBlock() != Blocks.AIR) {
				AxisAlignedBB axisalignedbb = iblockstate.getSelectedBoundingBox(world, blockpos).expandXyz(0.002D)
						.offset(-d0, -d1, -d2);
