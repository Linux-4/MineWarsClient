...
public class DebugRendererNeighborsUpdate implements DebugRenderer.IDebugRenderer {
	...
	public void render(float p_190060_1_, long p_190060_2_) {
				...
				for (Entry<BlockPos, Integer> entry1 : map1.entrySet()) {
					...
					if (set.add(blockpos)) {
						RenderGlobal.drawSelectionBoundingBox(
								(new AxisAlignedBB(BlockPos.ORIGIN)).expandXyz(0.002D).contract(0.0025D * (double) k)
										.offset((double) blockpos.getX(), (double) blockpos.getY(),
		...
		for (Entry<BlockPos, Integer> entry2 : map.entrySet()) {
			...
			DebugRenderer.func_191556_a(String.valueOf((Object) integer1), blockpos1.getX(), blockpos1.getY(),
					blockpos1.getZ(), p_190060_1_, -1);
		}
