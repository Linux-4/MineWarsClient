...
public class DebugRendererPathfinding implements DebugRenderer.IDebugRenderer {
		...
		if (!this.pathMap.isEmpty()) {
			...
			for (Integer integer : this.pathMap.keySet()) {
				...
				if (this.addDistanceToPlayer(pathpoint) <= 40.0F) {
					RenderGlobal.renderFilledBox((new AxisAlignedBB((double) ((float) pathpoint.xCoord + 0.25F),
							(double) ((float) pathpoint.yCoord + 0.25F), (double) pathpoint.zCoord + 0.25D,
							(double) ((float) pathpoint.xCoord + 0.75F), (double) ((float) pathpoint.yCoord + 0.75F),
							(double) ((float) pathpoint.zCoord + 0.75F))).offset(-this.xo, -this.yo, -this.zo), 0.0F,
							1.0F, 0.0F, 0.5F);
					...
					for (int j = 0; j < path.getCurrentPathLength(); ++j) {
						...
						if (this.addDistanceToPlayer(pathpoint1) <= 40.0F) {
							...
							RenderGlobal
									.renderFilledBox((new AxisAlignedBB((double) ((float) pathpoint1.xCoord + 0.5F - f),
											(double) ((float) pathpoint1.yCoord + 0.01F * (float) j),
											(double) ((float) pathpoint1.zCoord + 0.5F - f),
											(double) ((float) pathpoint1.xCoord + 0.5F + f),
											(double) ((float) pathpoint1.yCoord + 0.25F + 0.01F * (float) j),
											(double) ((float) pathpoint1.zCoord + 0.5F + f))).offset(-this.xo, -this.yo,
													-this.zo),
											f1, 0.0F, f2, 0.5F);
						}
					}
			...
			for (Integer integer1 : this.pathMap.keySet()) {
				...
				for (PathPoint pathpoint3 : path1.getClosedSet()) {
					if (this.addDistanceToPlayer(pathpoint3) <= 40.0F) {
						DebugRenderer.renderDebugText(String.format("%s", pathpoint3.nodeType),
								(double) pathpoint3.xCoord + 0.5D, (double) pathpoint3.yCoord + 0.75D,
								(double) pathpoint3.zCoord + 0.5D, p_190060_1_, -65536);
						DebugRenderer.renderDebugText(String.format("%.2f", pathpoint3.costMalus),
								(double) pathpoint3.xCoord + 0.5D, (double) pathpoint3.yCoord + 0.25D,
								(double) pathpoint3.zCoord + 0.5D, p_190060_1_, -65536);
					}
				}
				...
				for (PathPoint pathpoint4 : path1.getOpenSet()) {
					if (this.addDistanceToPlayer(pathpoint4) <= 40.0F) {
						DebugRenderer.renderDebugText(String.format("%s", pathpoint4.nodeType),
								(double) pathpoint4.xCoord + 0.5D, (double) pathpoint4.yCoord + 0.75D,
								(double) pathpoint4.zCoord + 0.5D, p_190060_1_, -16776961);
						DebugRenderer.renderDebugText(String.format("%.2f", pathpoint4.costMalus),
								(double) pathpoint4.xCoord + 0.5D, (double) pathpoint4.yCoord + 0.25D,
								(double) pathpoint4.zCoord + 0.5D, p_190060_1_, -16776961);
					}
				}
				...
				for (int k = 0; k < path1.getCurrentPathLength(); ++k) {
					...
					if (this.addDistanceToPlayer(pathpoint2) <= 40.0F) {
						DebugRenderer.renderDebugText(String.format("%s", pathpoint2.nodeType),
								(double) pathpoint2.xCoord + 0.5D, (double) pathpoint2.yCoord + 0.75D,
								(double) pathpoint2.zCoord + 0.5D, p_190060_1_, -1);
						DebugRenderer.renderDebugText(String.format("%.2f", pathpoint2.costMalus),
								(double) pathpoint2.xCoord + 0.5D, (double) pathpoint2.yCoord + 0.25D,
								(double) pathpoint2.zCoord + 0.5D, p_190060_1_, -1);
					}
				}
			}
	...
	public void renderPathLine(float p_190067_1_, Path p_190067_2_) {
		...
		for (int i = 0; i < p_190067_2_.getCurrentPathLength(); ++i) {
			...
			if (this.addDistanceToPlayer(pathpoint) <= 40.0F) {
				...
				int l = j >> 8 & 255;
				int i1 = j & 255;
				bufferbuilder.pos((double) pathpoint.xCoord - this.xo + 0.5D,
						(double) pathpoint.yCoord - this.yo + 0.5D, (double) pathpoint.zCoord - this.zo + 0.5D)
						.color(k, l, i1, 255).endVertex();
			}
		}
	...
	private float addDistanceToPlayer(PathPoint p_190066_1_) {
		return (float) (Math.abs((double) p_190066_1_.xCoord - this.player.posX)
				+ Math.abs((double) p_190066_1_.yCoord - this.player.posY)
				+ Math.abs((double) p_190066_1_.zCoord - this.player.posZ));
	}
}
