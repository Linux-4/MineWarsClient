	...
	public void draw(BufferBuilder vertexBufferIn) {
		if (vertexBufferIn.getVertexCount() > 0) {
			...
			for (int j = 0; j < list.size(); ++j) {
				...
				if (flag) {
					Reflector.callVoid(vertexformatelement$enumusage,
							Reflector.ForgeVertexFormatElementEnumUseage_preDraw, vertexformat, j, i, bytebuffer);
				} else {
			...
			if (vertexBufferIn.isMultiTexture()) {
				...
				SVertexBuilder.drawArrays(vertexBufferIn.getDrawMode(), 0, vertexBufferIn.getVertexCount(),
						vertexBufferIn);
			} else {
			...
			for (int k1 = list.size(); j1 < k1; ++j1) {
				...
				if (flag1) {
					Reflector.callVoid(vertexformatelement$enumusage1,
							Reflector.ForgeVertexFormatElementEnumUseage_postDraw, vertexformat, j1, i, bytebuffer);
				} else {
