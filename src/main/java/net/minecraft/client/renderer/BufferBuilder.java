...
public class BufferBuilder {
	...
	private void growBuffer(int p_181670_1_) {
		...
		if (MathHelper.roundUp(p_181670_1_, 4) / 4 > this.rawIntBuffer.remaining()
				|| this.vertexCount * this.vertexFormat.getNextOffset() + p_181670_1_ > this.byteBuffer.capacity()) {
	...
	/**
	 * Gets the position into the vertex data buffer at which the given vertex's
	 * color data can be found, in {@code
	 * int}s.
	 * 
	 * @param vertexIndex The index of the vertex in question, where 0 is the last
	 *                    one added, 1 is the second last, etc.
	 */
	public int getColorIndex(int vertexIndex) {
		return ((this.vertexCount - vertexIndex) * this.vertexFormat.getNextOffset()
	 ...
	 * 
	 * @param vertexIndex The index of the vertex to modify, where 0 is the last one
	 *                    added, 1 is the second last, etc.
	 */
	...
	/**
	 * Write the given color data of 4 bytes at the given index into the vertex data
	 * buffer, accounting for system endianness.
	 * 
	 * @param index The index in the vertex data buffer to which the color data will
	 *              be written, in {@code int}s
	 */
	...
	public RenderEnv getRenderEnv(IBlockAccess p_getRenderEnv_1_, IBlockState p_getRenderEnv_2_,
			BlockPos p_getRenderEnv_3_) {
	...
	public void putColorRGBA(int p_putColorRGBA_1_, int p_putColorRGBA_2_, int p_putColorRGBA_3_, int p_putColorRGBA_4_,
			int p_putColorRGBA_5_) {
		if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
			this.rawIntBuffer.put(p_putColorRGBA_1_,
					p_putColorRGBA_5_ << 24 | p_putColorRGBA_4_ << 16 | p_putColorRGBA_3_ << 8 | p_putColorRGBA_2_);
		} else {
			this.rawIntBuffer.put(p_putColorRGBA_1_,
					p_putColorRGBA_2_ << 24 | p_putColorRGBA_3_ << 16 | p_putColorRGBA_4_ << 8 | p_putColorRGBA_5_);
		}
	}
