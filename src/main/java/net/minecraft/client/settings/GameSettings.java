...
public class GameSettings {
	...
	public static final String[] field_193632_b = new String[] { "options.narrator.off", "options.narrator.all",
	...
	public GameSettings(Minecraft mcIn, File optionsFileIn) {
		...
		this.keyBindings = (KeyBinding[]) ArrayUtils.addAll(new KeyBinding[] { this.keyBindAttack, this.keyBindUseItem,
				...
				this.keyBindSpectatorOutlines, this.keyBindSwapHands, this.field_193629_ap, this.field_193630_aq,
		...
		if (s != null) {
			...
			if (settingOption.getEnumFloat()) {
				...
				if (settingOption == GameSettings.Options.SENSITIVITY) {
					...
					return f1 == settingOption.valueMax ? s1 + I18n.format("options.framerateLimit.max")
							: s1 + I18n.format("options.framerate", (int) f1);
				} else if (settingOption == GameSettings.Options.RENDER_CLOUDS) {
					return f1 == settingOption.valueMin ? s1 + I18n.format("options.cloudHeight.min")
							: s1 + ((int) f1 + 128);
				} else if (settingOption == GameSettings.Options.GAMMA) {
					if (f == 0.0F) {
				...
				return NarratorChatListener.field_193643_a.func_193640_a()
						? s1 + getTranslation(field_193632_b, this.field_192571_R)
						: s1 + I18n.format("options.narrator.notavailable");
			} else {
				return s1;
			}
		}
						...
						if (s1.equals("key_" + keybinding.getKeyDescription())) {
							if (Reflector.KeyModifier_valueFromString.exists()) {
								if (s2.indexOf(58) != -1) {
									...
									Reflector.call(keybinding, Reflector.ForgeKeyBinding_setKeyModifierAndCode, object,
											Integer.parseInt(astring[0]));
								} else {
									Object object1 = Reflector.getFieldValue(Reflector.KeyModifier_NONE);
									Reflector.call(keybinding, Reflector.ForgeKeyBinding_setKeyModifierAndCode, object1,
											Integer.parseInt(s2));
								}
							} else {
	...
	public void sendSettingsToServer() {
		if (this.mc.player != null) {
			...
			this.mc.player.connection.sendPacket(new CPacketClientSettings(this.language, this.renderDistanceChunks,
					this.chatVisibility, this.chatColours, i, this.mainHand));
		}
	}
	...
	private float getOptionFloatValueOF(GameSettings.Options p_getOptionFloatValueOF_1_) {
		if (p_getOptionFloatValueOF_1_ == GameSettings.Options.CLOUD_HEIGHT) {
			...
			return (float) this.limitFramerate == GameSettings.Options.FRAMERATE_LIMIT.getValueMax() && this.enableVsync
					? 0.0F
	...
	private String getKeyBindingOF(GameSettings.Options p_getKeyBindingOF_1_) {
		...
		if (p_getKeyBindingOF_1_ == GameSettings.Options.RENDER_DISTANCE) {
			...
			if (this.ofAutoSaveTicks <= 40) {
				...
				return this.ofAutoSaveTicks <= 4000 ? s + Lang.get("of.options.save.3min")
			...
			if (f == 0.0F) {
				...
				return f == p_getKeyBindingOF_1_.valueMax ? s + I18n.format("options.framerateLimit.max")
						: s + (int) f + " fps";
			}
		} else {
			return null;
	...
	public void saveOfOptions() {
		try {
			PrintWriter printwriter = new PrintWriter(
					new OutputStreamWriter(new FileOutputStream(this.optionsFileOF), StandardCharsets.UTF_8));
	...
	public static enum Options {
		...
		REALMS_NOTIFICATIONS("options.realmsNotifications", false, true), AUTO_JUMP("options.autoJump", false, true),
		NARRATOR("options.narrator", false, false), FOG_FANCY("of.options.FOG_FANCY", false, false),
		...
		SWAMP_COLORS("of.options.SWAMP_COLORS", false, false), RANDOM_MOBS("of.options.RANDOM_MOBS", false, false),
