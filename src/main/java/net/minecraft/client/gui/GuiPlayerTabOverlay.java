...
public class GuiPlayerTabOverlay extends Gui {
	...
	public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn) {
		...
		for (NetworkPlayerInfo networkplayerinfo : list) {
			...
			if (scoreObjectiveIn != null && scoreObjectiveIn.getRenderType() != IScoreCriteria.EnumRenderType.HEARTS) {
				k = this.mc.fontRendererObj.getStringWidth(" "
						+ scoreboardIn.getOrCreateScore(networkplayerinfo.getGameProfile().getName(), scoreObjectiveIn)
								.getScorePoints());
				j = Math.max(j, k);
			}
		}
		...
		for (int k4 = 0; k4 < l3; ++k4) {
			...
			if (k4 < list.size()) {
				...
				if (networkplayerinfo1.getGameType() == GameType.SPECTATOR) {
					this.mc.fontRendererObj.drawStringWithShadow(TextFormatting.ITALIC + s4, (float) j2, (float) k2,
							-1862270977);
				} else {
	...
	private void drawScoreboardValues(ScoreObjective objective, int p_175247_2_, String name, int p_175247_4_,
		...
		if (objective.getRenderType() == IScoreCriteria.EnumRenderType.HEARTS) {
			...
			int k = Math.max(MathHelper.ceil((float) (i / 2)),
