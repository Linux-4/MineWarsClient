	...
	protected List<String> call() {
		...
		if (this.mc.isReducedDebug()) {
			return Lists.newArrayList(
					...
					this.mc.world.getProviderName(), "", String.format("Chunk-relative: %d %d %d", blockpos.getX() & 15,
			...
			List<String> list = Lists.newArrayList(
					...
					String.format("Facing: %s (%s) (%.1f / %.1f)", enumfacing, s,
							MathHelper.wrapDegrees(entity.rotationYaw), MathHelper.wrapDegrees(entity.rotationPitch)));
				...
				if (this.mc.world.isBlockLoaded(blockpos) && blockpos.getY() >= 0 && blockpos.getY() < 256) {
					if (!chunk.isEmpty()) {
						...
						if (this.mc.isIntegratedServerRunning() && this.mc.getIntegratedServer() != null) {
							EntityPlayerMP entityplayermp = this.mc.getIntegratedServer().getPlayerList()
						...
						list.add(String.format("Local Difficulty: %.2f // %.2f (Day %d)",
