...
public class GuiContainerCreative extends InventoryEffectRenderer {
			...
			if (slotIn == this.destroyItemSlot && flag) {
				...
				if (slotIn == this.destroyItemSlot) {
					...
					ItemStack itemstack = slotIn
							.decrStackSize(mouseButton == 0 ? 1 : slotIn.getStack().getMaxStackSize());
				...
				if (!itemstack5.func_190926_b() && !itemstack7.func_190926_b() && itemstack5.isItemEqual(itemstack7)
				...
				ItemStack itemstack3 = slotIn == null ? ItemStack.field_190927_a
						: this.inventorySlots.getSlot(slotIn.slotNumber).getStack();
				this.inventorySlots.slotClick(slotIn == null ? slotId : slotIn.slotNumber, mouseButton, type,
						this.mc.player);
				...
				if (Container.getDragEvent(mouseButton) == 2) {
					...
					if (type == ClickType.SWAP) {
						this.mc.playerController.sendSlotPacket(itemstack3,
								i - this.inventorySlots.inventorySlots.size() + 9 + 36);
	...
	public void initGui() {
		if (this.mc.playerController.isInCreativeMode()) {
			...
			this.searchField = new GuiTextField(0, this.fontRendererObj, this.guiLeft + 82, this.guiTop + 6, 80,
	...
	private void updateCreativeSearch() {
		...
		if (this.searchField.getText().isEmpty()) {
			...
			guicontainercreative$containercreative.itemList
					.addAll(this.mc.func_193987_a(SearchTreeManager.field_194011_a)
							.func_194038_a(this.searchField.getText().toLowerCase(Locale.ROOT)));
		}
					...
					for (int k = 0; k < 9; ++k) {
						if (k == j) {
							...
							String s1 = GameSettings
									.getKeyDisplayString(this.mc.gameSettings.field_193629_ap.getKeyCode());
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.destroyItemSlot != null && selectedTabIndex == CreativeTabs.INVENTORY.getTabIndex()
				&& this.isPointInRegion(this.destroyItemSlot.xDisplayPosition, this.destroyItemSlot.yDisplayPosition,
						16, 16, mouseX, mouseY)) {
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		if (creativetabs == CreativeTabs.INVENTORY) {
			GuiInventory.drawEntityOnScreen(this.guiLeft + 88, this.guiTop + 45, 20,
					(float) (this.guiLeft + 88 - mouseX), (float) (this.guiTop + 45 - 30 - mouseY), this.mc.player);
		}
	}
	...
	/**
	 * Draws the given tab and its background, deciding whether to highlight the tab
	 * or not based off of the selected index.
	 */
	...
	public static void func_192044_a(Minecraft p_192044_0_, int p_192044_1_, boolean p_192044_2_, boolean p_192044_3_) {
		...
		if (p_192044_2_) {
			...
			p_192044_0_.ingameGUI.setRecordPlaying(
					new TextComponentTranslation("inventory.hotbarSaved", new Object[] { s1, s }), false);
	...
	public static class ContainerCreative extends Container {
		...
		public ContainerCreative(EntityPlayer player) {
			...
			for (int i = 0; i < 5; ++i) {
				for (int j = 0; j < 9; ++j) {
					this.addSlotToContainer(new GuiContainerCreative.LockedSlot(GuiContainerCreative.basicInventory,
							i * 9 + j, 9 + j * 18, 18 + i * 18));
				}
			}
			...
			for (int k = 0; k < 5; ++k) {
				for (int l = 0; l < 9; ++l) {
					...
					if (i1 >= 0 && i1 < this.itemList.size()) {
						...
						GuiContainerCreative.basicInventory.setInventorySlotContents(l + k * 9,
		...
		public boolean canDragIntoSlot(Slot slotIn) {
			return slotIn.inventory instanceof InventoryPlayer
					|| slotIn.yDisplayPosition > 90 && slotIn.xDisplayPosition <= 162;
		}
	}
