...
public class GuiBeacon extends GuiContainer {
		...
		if (this.buttonsNotDrawn && i >= 0) {
			...
			for (int k = 0; k <= 2; ++k) {
				...
				for (int j1 = 0; j1 < l; ++j1) {
					...
					GuiBeacon.PowerButton guibeacon$powerbutton = new GuiBeacon.PowerButton(j++,
			...
			for (int j2 = 0; j2 < l1 - 1; ++j2) {
				...
				GuiBeacon.PowerButton guibeacon$powerbutton2 = new GuiBeacon.PowerButton(j++,
			...
			if (potion != null) {
				GuiBeacon.PowerButton guibeacon$powerbutton1 = new GuiBeacon.PowerButton(j++,
	...
	class PowerButton extends GuiBeacon.Button {
		...
		public PowerButton(int buttonId, int x, int y, Potion effectIn, int tierIn) {
			super(buttonId, x, y, GuiContainer.INVENTORY_BACKGROUND, effectIn.getStatusIconIndex() % 8 * 18,
