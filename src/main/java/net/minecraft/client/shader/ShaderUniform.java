...
public class ShaderUniform {
	...
	public void set(float[] p_148097_1_) {
		if (p_148097_1_.length < this.uniformCount) {
			LOGGER.warn("Uniform.set called with a too-small value array (expected {}, got {}). Ignoring.",
					Integer.valueOf(this.uniformCount), Integer.valueOf(p_148097_1_.length));
		} else {
			this.uniformFloatBuffer.position(0);
	...
	public void set(Matrix4f matrix) {
		this.set(matrix.m00, matrix.m01, matrix.m02, matrix.m03, matrix.m10, matrix.m11, matrix.m12, matrix.m13,
	...
	public void upload() {
		...
		if (this.uniformType <= 3) {
			...
			if (this.uniformType > 10) {
				LOGGER.warn("Uniform.upload called, but type value ({}) is not a valid type. Ignoring.",
