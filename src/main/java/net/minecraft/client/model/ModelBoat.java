...
public class ModelBoat extends ModelBase implements IMultipassModel {
	...
	protected void renderPaddle(EntityBoat boat, int paddle, float scale, float limbSwing) {
		...
		modelrenderer.rotateAngleX = (float) MathHelper.clampedLerp(-1.0471975803375244D, -0.2617993950843811D,
				(double) ((MathHelper.sin(-f) + 1.0F) / 2.0F));
		modelrenderer.rotateAngleY = (float) MathHelper.clampedLerp(-(Math.PI / 4D), (Math.PI / 4D),
				(double) ((MathHelper.sin(-f + 1.0F) + 1.0F) / 2.0F));
