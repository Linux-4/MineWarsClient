...
public class MusicTicker implements ITickable {
	...
	public void update() {
		...
		if (this.currentMusic != null) {
			...
			if (!this.mc.getSoundHandler().isSoundPlaying(this.currentMusic)) {
				...
				this.timeUntilNextMusic = Math.min(MathHelper.getInt(this.rand, musicticker$musictype.getMinDelay(),
						musicticker$musictype.getMaxDelay()), this.timeUntilNextMusic);
			}
		}
	...
	public static enum MusicType {
		MENU(SoundEvents.MUSIC_MENU, 20, 600), GAME(SoundEvents.MUSIC_GAME, 12000, 24000),
		CREATIVE(SoundEvents.MUSIC_CREATIVE, 1200, 3600), CREDITS(SoundEvents.MUSIC_CREDITS, 0, 0),
		NETHER(SoundEvents.MUSIC_NETHER, 1200, 3600), END_BOSS(SoundEvents.MUSIC_DRAGON, 0, 0),
