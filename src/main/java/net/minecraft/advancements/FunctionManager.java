		...
		if (this.field_193068_b != null) {
			...
			for (File file1 : FileUtils.listFiles(this.field_193068_b, new String[] { "mcfunction" }, true)) {
				String s = FilenameUtils
						.removeExtension(this.field_193068_b.toURI().relativize(file1.toURI()).toString());
				...
				if (astring.length == 2) {
					...
					try {
						this.field_193070_d.put(resourcelocation,
								FunctionObject.func_193527_a(this, Files.readLines(file1, StandardCharsets.UTF_8)));
					} catch (Throwable throwable) {
						field_193067_a.error("Couldn't read custom function " + resourcelocation + " from " + file1,
								throwable);
					}
				}
			}
