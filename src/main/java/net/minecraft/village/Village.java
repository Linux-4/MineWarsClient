...
public class Village {
	...
	private Vec3d findRandomSpawnPos(BlockPos pos, int x, int y, int z) {
		for (int i = 0; i < 10; ++i) {
			BlockPos blockpos = pos.add(this.worldObj.rand.nextInt(16) - 8, this.worldObj.rand.nextInt(6) - 3,
	...
	/**
	 * Checks to see if the volume around blockLocation is clear and able to fit
	 * blockSize
	 */
	...
	/**
	 * Checks to see if the distance squared between this BlockPos and the center of
	 * this Village is less than this Village's villageRadius squared
	 */
	...
	public VillageDoorInfo getDoorInfo(BlockPos pos) {
		...
		for (VillageDoorInfo villagedoorinfo1 : this.villageDoorInfoList) {
			...
			if (j < i) {
				...
				if (this.worldObj.getBlockState(blockpos.offset(enumfacing, 1)).getBlock().isPassable(this.worldObj,
						blockpos.offset(enumfacing, 1))
						&& this.worldObj.getBlockState(blockpos.offset(enumfacing, -1)).getBlock()
								.isPassable(this.worldObj, blockpos.offset(enumfacing, -1))
						&& this.worldObj.getBlockState(blockpos.up().offset(enumfacing, 1)).getBlock()
								.isPassable(this.worldObj, blockpos.up().offset(enumfacing, 1))
						&& this.worldObj.getBlockState(blockpos.up().offset(enumfacing, -1)).getBlock()
								.isPassable(this.worldObj, blockpos.up().offset(enumfacing, -1))) {
					villagedoorinfo = villagedoorinfo1;
					i = j;
	...
	/**
	 * Modify a players reputation in the village. Use positive values to increase
	 * reputation and negative values to decrease. <br>
