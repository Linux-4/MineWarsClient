...
public class VillageSiege {
				...
				if (this.theVillage != null && this.theVillage.getNumVillageDoors() >= 10
					...
					for (int i = 0; i < 10; ++i) {
						...
						for (Village village : this.worldObj.getVillageCollection().getVillageList()) {
							if (village != this.theVillage && village.isBlockPosWithinSqVillageRadius(
	...
	private boolean spawnZombie() {
		...
		if (vec3d == null) {
			...
			entityzombie.setLocationAndAngles(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord,
	...
	private Vec3d findRandomSpawnPos(BlockPos pos) {
		for (int i = 0; i < 10; ++i) {
			...
			if (this.theVillage.isBlockPosWithinSqVillageRadius(blockpos)
