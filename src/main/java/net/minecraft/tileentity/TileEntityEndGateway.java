...
public class TileEntityEndGateway extends TileEntityEndPortal implements ITickable {
	...
	public void update() {
		...
		if (flag1) {
			...
			List<Entity> list = this.world.<Entity>getEntitiesWithinAABB(Entity.class,
	...
	public void teleportEntity(Entity entityIn) {
		if (!this.world.isRemote && !this.isCoolingDown()) {
			...
			if (this.exitPortal != null) {
				...
				entityIn.setPositionAndUpdate((double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.5D,
	...
	private static Chunk getChunk(World worldIn, Vec3d vec3) {
		return worldIn.getChunkFromChunkCoords(MathHelper.floor(vec3.xCoord / 16.0D),
				MathHelper.floor(vec3.zCoord / 16.0D));
	}
	...
	private static BlockPos findSpawnpointInChunk(Chunk chunkIn) {
		...
		for (BlockPos blockpos3 : BlockPos.getAllInBox(blockpos, blockpos1)) {
			...
			if (iblockstate.getBlock() == Blocks.END_STONE
					&& !chunkIn.getBlockState(blockpos3.up(1)).isBlockNormalCube()
					&& !chunkIn.getBlockState(blockpos3.up(2)).isBlockNormalCube()) {
