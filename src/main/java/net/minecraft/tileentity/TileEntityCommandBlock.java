...
public class TileEntityCommandBlock extends TileEntity {
	...
	private final CommandBlockBaseLogic commandBlockLogic = new CommandBlockBaseLogic() {
		...
		public Vec3d getPositionVector() {
			return new Vec3d((double) TileEntityCommandBlock.this.pos.getX() + 0.5D,
		...
		public void updateCommand() {
			...
			TileEntityCommandBlock.this.getWorld().notifyBlockUpdate(TileEntityCommandBlock.this.pos, iblockstate,
					iblockstate, 3);
		}
	...
	public void setAuto(boolean autoIn) {
		...
		if (!flag && autoIn && !this.powered && this.world != null
	...
	public boolean setConditionMet() {
		...
		if (this.isConditional()) {
			BlockPos blockpos = this.pos.offset(
					((EnumFacing) this.world.getBlockState(this.pos).getValue(BlockCommandBlock.FACING)).getOpposite());
