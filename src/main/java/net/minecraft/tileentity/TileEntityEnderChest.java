...
public class TileEntityEnderChest extends TileEntity implements ITickable {
	...
	public void update() {
		...
		if (this.numPlayersUsing > 0 && this.lidAngle == 0.0F) {
			...
			this.world.playSound((EntityPlayer) null, d0, (double) j + 0.5D, d1, SoundEvents.BLOCK_ENDERCHEST_OPEN,
		...
		if (this.numPlayersUsing == 0 && this.lidAngle > 0.0F || this.numPlayersUsing > 0 && this.lidAngle < 1.0F) {
			...
			if (this.lidAngle < 0.5F && f2 >= 0.5F) {
				...
				this.world.playSound((EntityPlayer) null, d3, (double) j + 0.5D, d2, SoundEvents.BLOCK_ENDERCHEST_CLOSE,
						SoundCategory.BLOCKS, 0.5F, this.world.rand.nextFloat() * 0.1F + 0.9F);
			}
