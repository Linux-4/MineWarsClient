...
public abstract class CommandBlockBaseLogic implements ICommandSender {
		...
		if (!worldIn.isRemote && worldIn.getTotalWorldTime() != this.field_193041_b) {
			if ("Searge".equalsIgnoreCase(this.commandStored)) {
				...
				if (minecraftserver != null && minecraftserver.isAnvilFileSet()
					...
					try {
						this.lastOutput = null;
						this.successCount = minecraftserver.getCommandManager().executeCommand(this,
								this.commandStored);
					} catch (Throwable throwable) {
	...
	/**
	 * Currently this returns 0 for the traditional command block, and 1 for the
	 * minecart command block
	 */
	public abstract int getCommandBlockType();

	/**
	 * Fills in information about the command block for the packet. X/Y/Z for the
	 * minecart version, and entityId for the traditional version
	 */
