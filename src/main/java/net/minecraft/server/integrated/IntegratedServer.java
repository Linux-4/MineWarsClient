...
public class IntegratedServer extends MinecraftServer {
	...
	public IntegratedServer(Minecraft clientIn, String folderNameIn, String worldNameIn, WorldSettings worldSettingsIn,
		...
		super(new File(clientIn.mcDataDir, "saves"), clientIn.getProxy(), clientIn.getDataFixer(), authServiceIn,
				sessionServiceIn, profileRepoIn, profileCacheIn);
	...
	public void loadAllWorlds(String saveName, String worldNameIn, long seed, WorldType type, String generatorOptions) {
		...
		if (Reflector.DimensionManager.exists()) {
			WorldServer worldserver = this.isDemo()
					? (WorldServer) ((WorldServer) (new WorldServerDemo(this, isavehandler, worldinfo, 0,
			...
			for (int l = 0; l < this.worldServers.length; ++l) {
				...
				if (l == 0) {
					if (this.isDemo()) {
						this.worldServers[l] = (WorldServer) (new WorldServerDemo(this, isavehandler, worldinfo, i1,
								this.theProfiler)).init();
					} else {
						this.worldServers[l] = (WorldServer) (new WorldServerOF(this, isavehandler, worldinfo, i1,
								this.theProfiler)).init();
					}

					this.worldServers[l].initialize(this.theWorldSettings);
				} else {
					this.worldServers[l] = (WorldServer) (new WorldServerMulti(this, isavehandler, i1,
	...
	public boolean startServer() throws IOException {
		...
		this.loadAllWorlds(this.getFolderName(), this.getWorldName(), this.theWorldSettings.getSeed(),
				this.theWorldSettings.getTerrainType(), this.theWorldSettings.getGeneratorOptions());
		...
		if (this.isGamePaused) {
			...
			if (this.mc.gameSettings.renderDistanceChunks != this.getPlayerList().getViewDistance()) {
				LOGGER.info("Changing view distance to {}, from {}",
						...
						Integer.valueOf(this.getPlayerList().getViewDistance()));
	...
	/**
	 * Get if native transport should be used. Native transport means linux server
	 * performance improvements and optimized packet sending/receiving on linux
	 */
			...
			Futures.getUnchecked(this.addScheduledTask(new Runnable() {
				public void run() {
					for (EntityPlayerMP entityplayermp : Lists
							.newArrayList(IntegratedServer.this.getPlayerList().getPlayerList())) {
