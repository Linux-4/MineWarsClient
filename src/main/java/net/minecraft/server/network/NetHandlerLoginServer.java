...
public class NetHandlerLoginServer implements INetHandlerLoginServer, ITickable {
	...
	public void tryAcceptPlayer() {
		...
		String s = this.server.getPlayerList().allowUserToConnect(this.networkManager.getRemoteAddress(),
		...
		if (s != null) {
			...
			if (entityplayermp != null) {
				...
				this.server.getPlayerList().initializeConnectionToPlayer(this.networkManager,
						this.server.getPlayerList().createPlayerForUser(this.loginGameProfile));
			}
		}
	}
	...
	public void processLoginStart(CPacketLoginStart packetIn) {
		...
		if (this.server.isServerInOnlineMode() && !this.networkManager.isLocalChannel()) {
			...
			this.networkManager.sendPacket(
					new SPacketEncryptionRequest("", this.server.getKeyPair().getPublic(), this.verifyToken));
		} else {
			this.currentLoginState = NetHandlerLoginServer.LoginState.READY_TO_ACCEPT;
		}
	}
		...
		if (!Arrays.equals(this.verifyToken, packetIn.getVerifyToken(privatekey))) {
			...
			(new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet()) {
				public void run() {
					...
					try {
						String s = (new BigInteger(CryptManager.getServerIdHash("",
						...
						NetHandlerLoginServer.this.loginGameProfile = NetHandlerLoginServer.this.server
								.getMinecraftSessionService().hasJoinedServer(
										new GameProfile((UUID) null, gameprofile.getName()), s, this.func_191235_a());
						...
						if (NetHandlerLoginServer.this.loginGameProfile != null) {
							...
							NetHandlerLoginServer.this.func_194026_b(new TextComponentTranslation(
									"multiplayer.disconnect.unverified_username", new Object[0]));
							NetHandlerLoginServer.LOGGER.error("Username '{}' tried to join with an invalid session",
						...
						if (NetHandlerLoginServer.this.server.isSinglePlayer()) {
							...
							NetHandlerLoginServer.this.func_194026_b(new TextComponentTranslation(
									"multiplayer.disconnect.authservers_down", new Object[0]));
							NetHandlerLoginServer.LOGGER
				...
				private InetAddress func_191235_a() {
					...
					return NetHandlerLoginServer.this.server.func_190518_ac()
