...
public class PlayerChunkMapEntry {
	...
	public void addPlayer(EntityPlayerMP player) {
		if (this.players.contains(player)) {
			LOGGER.debug("Failed to add player. {} already is in chunk {}, {}", player,
					Integer.valueOf(this.pos.chunkXPos), Integer.valueOf(this.pos.chunkZPos));
		} else {
	...
	/**
	 * Provide the chunk at the player's location. Can fail, returning false, if the
	 * player is a spectator floating outside of any pre-existing chunks, and the
	 * server is not configured to allow chunk generation for spectators.
	 */
	public boolean providePlayerChunk(boolean canGenerate) {
		if (this.chunk != null) {
			...
			if (canGenerate) {
				this.chunk = this.playerChunkMap.getWorldServer().getChunkProvider().provideChunk(this.pos.chunkXPos,
						this.pos.chunkZPos);
			} else {
				this.chunk = this.playerChunkMap.getWorldServer().getChunkProvider().loadChunk(this.pos.chunkXPos,
						this.pos.chunkZPos);
			}
	...
	public boolean sendToPlayers() {
		if (this.sentToPlayers) {
			...
			for (EntityPlayerMP entityplayermp : this.players) {
				...
				this.playerChunkMap.getWorldServer().getEntityTracker().sendLeashedEntitiesInChunk(entityplayermp,
						this.chunk);
			}
	...
	/**
	 * Send packets to player for: - nearby tile entities - nearby entities that are
	 * leashed - nearby entities with
	 */
