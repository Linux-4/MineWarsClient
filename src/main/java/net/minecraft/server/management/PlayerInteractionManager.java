...
public class PlayerInteractionManager {
	...
	public void setGameType(GameType type) {
		...
		this.thisPlayerMP.mcServer.getPlayerList().sendPacketToAllPlayers(new SPacketPlayerListItem(
				SPacketPlayerListItem.Action.UPDATE_GAME_MODE, new EntityPlayerMP[] { this.thisPlayerMP }));
	...
	public void updateBlockRemoving() {
		...
		if (this.receivedFinishDiggingPacket) {
			...
			if (iblockstate.getMaterial() == Material.AIR) {
				...
				float f = iblockstate.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.world,
						this.delayedDestroyPos) * (float) (i + 1);
				int j = (int) (f * 10.0F);
			...
			if (iblockstate1.getMaterial() == Material.AIR) {
				...
				float f1 = iblockstate1.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.world,
						this.delayedDestroyPos) * (float) (k + 1);
				int l = (int) (f1 * 10.0F);
	...
	public void blockRemoving(BlockPos pos) {
		if (pos.equals(this.destroyPos)) {
			...
			if (iblockstate.getMaterial() != Material.AIR) {
				float f = iblockstate.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.world, pos)
						* (float) (i + 1);
	...
	public boolean tryHarvestBlock(BlockPos pos) {
		if (this.gameType.isCreative() && !this.thisPlayerMP.getHeldItemMainhand().func_190926_b()
				&& this.thisPlayerMP.getHeldItemMainhand().getItem() instanceof ItemSword) {
			return false;
		} else {
			IBlockState iblockstate = this.theWorld.getBlockState(pos);
			...
			if ((block instanceof BlockCommandBlock || block instanceof BlockStructure)
					&& !this.thisPlayerMP.canUseCommandBlock()) {
				...
				if (this.isCreative()) {
					...
					if (flag1 && flag) {
						iblockstate.getBlock().harvestBlock(this.theWorld, this.thisPlayerMP, pos, iblockstate,
								tileentity, itemstack2);
					}
				}
	...
	public EnumActionResult processRightClick(EntityPlayer player, World worldIn, ItemStack stack, EnumHand hand) {
		if (this.gameType == GameType.SPECTATOR) {
			...
			if (itemstack == stack && itemstack.func_190916_E() == i && itemstack.getMaxItemUseDuration() <= 0
	...
	public EnumActionResult processRightClickBlock(EntityPlayer player, World worldIn, ItemStack stack, EnumHand hand,
		...
		if (this.gameType == GameType.SPECTATOR) {
			...
			if (!player.isSneaking()
				...
				if (iblockstate.getBlock().onBlockActivated(worldIn, pos, iblockstate, player, hand, facing, hitX, hitY,
						hitZ)) {
					return EnumActionResult.SUCCESS;
				}
			}
			...
			if (stack.func_190926_b()) {
				...
				if (this.isCreative()) {
					int j = stack.getMetadata();
					int i = stack.func_190916_E();
					EnumActionResult enumactionresult = stack.onItemUse(player, worldIn, pos, hand, facing, hitX, hitY,
							hitZ);
