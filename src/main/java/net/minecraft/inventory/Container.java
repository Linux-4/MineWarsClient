...
public abstract class Container {
		...
		if (clickTypeIn == ClickType.QUICK_CRAFT) {
			...
			if ((j1 != 1 || this.dragEvent != 2) && j1 != this.dragEvent) {
				...
				if (slot7 != null && canAddItemToSlot(slot7, itemstack12, true) && slot7.isItemValid(itemstack12)
						&& (this.dragMode == 2 || itemstack12.func_190916_E() > this.dragSlots.size())
				...
				if (!this.dragSlots.isEmpty()) {
					...
					for (Slot slot8 : this.dragSlots) {
						...
						if (slot8 != null && canAddItemToSlot(slot8, itemstack13, true)
								...
								&& (this.dragMode == 2 || itemstack13.func_190916_E() >= this.dragSlots.size())
			...
			if (slotId == -999) {
				...
				for (ItemStack itemstack7 = this.transferStackInSlot(player, slotId); !itemstack7.func_190926_b()
					...
					if (itemstack8.func_190926_b()) {
						...
						if (itemstack11.func_190926_b()) {
							if (itemstack8.func_190926_b()) {
								...
								int l2 = dragType == 0 ? itemstack8.func_190916_E()
										: (itemstack8.func_190916_E() + 1) / 2;
			...
			if (!itemstack6.func_190926_b() || !itemstack10.func_190926_b()) {
				...
				&& inventoryplayer.getItemStack().func_190926_b() && slotId >= 0) {
			...
			if (!itemstack1.func_190926_b() && (slot == null || !slot.getHasStack() || !slot.canTakeStack(player))) {
				...
				for (int k = 0; k < 2; ++k) {
					for (int l = i; l >= 0 && l < this.inventorySlots.size()
							&& itemstack1.func_190916_E() < itemstack1.getMaxStackSize(); l += j) {
						...
						if (slot1.getHasStack() && canAddItemToSlot(slot1, itemstack1, true)
							...
							if (k != 0 || itemstack2.func_190916_E() != itemstack2.getMaxStackSize()) {
								int i1 = Math.min(itemstack1.getMaxStackSize() - itemstack1.func_190916_E(),
	...
	protected void func_193327_a(EntityPlayer p_193327_1_, World p_193327_2_, IInventory p_193327_3_) {
		if (!p_193327_1_.isEntityAlive()
				|| p_193327_1_ instanceof EntityPlayerMP && ((EntityPlayerMP) p_193327_1_).func_193105_t()) {
	...
	protected boolean mergeItemStack(ItemStack stack, int startIndex, int endIndex, boolean reverseDirection) {
		...
		if (stack.isStackable()) {
			while (!stack.func_190926_b()) {
				...
				if (!itemstack.func_190926_b() && itemstack.getItem() == stack.getItem()
	...
	public static boolean canAddItemToSlot(@Nullable Slot slotIn, ItemStack stack, boolean stackSizeMatters) {
		...
		if (!flag && stack.isItemEqual(slotIn.getStack())
			...
			return slotIn.getStack().func_190916_E() + (stackSizeMatters ? 0 : stack.func_190916_E()) <= stack
					.getMaxStackSize();
		} else {
			return flag;
		}
		...
		if (inv == null) {
			...
			for (int j = 0; j < inv.getSizeInventory(); ++j) {
				...
				if (!itemstack.func_190926_b()) {
					f += (float) itemstack.func_190916_E()
	...
	protected void func_192389_a(World p_192389_1_, EntityPlayer p_192389_2_, InventoryCrafting p_192389_3_,
		...
		if (!p_192389_1_.isRemote) {
			...
			if (irecipe != null
					&& (irecipe.func_192399_d() || !p_192389_1_.getGameRules().getBoolean("doLimitedCrafting")
