...
public class ContainerHorseInventory extends Container {
		...
		if (horse instanceof AbstractChestHorse && ((AbstractChestHorse) horse).func_190695_dh()) {
			for (int k = 0; k < 3; ++k) {
				for (int l = 0; l < ((AbstractChestHorse) horse).func_190696_dl(); ++l) {
					this.addSlotToContainer(new Slot(horseInventoryIn,
							2 + l + k * ((AbstractChestHorse) horse).func_190696_dl(), 80 + l * 18, 18 + k * 18));
				}
			}
		}
	...
	public boolean canInteractWith(EntityPlayer playerIn) {
		return this.horseInventory.isUsableByPlayer(playerIn) && this.theHorse.isEntityAlive()
