...
public class AxisAlignedBB {
	...
	/**
	 * Adds a coordinate to the bounding box, extending it if the point lies outside
	 * the current ranges.
	 */
	...
	/**
	 * Creates a new bounding box that has been expanded. If negative values are
	 * used, it will shrink.
	 */
	...
	public boolean intersects(Vec3d min, Vec3d max) {
		return this.intersects(Math.min(min.xCoord, max.xCoord), Math.min(min.yCoord, max.yCoord),
				Math.min(min.zCoord, max.zCoord), Math.max(min.xCoord, max.xCoord), Math.max(min.yCoord, max.yCoord),
				Math.max(min.zCoord, max.zCoord));
	}
