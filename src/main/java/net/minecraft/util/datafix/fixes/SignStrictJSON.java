				...
				public ITextComponent deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_,
					...
					if (p_deserialize_1_.isJsonPrimitive()) {
						...
						for (JsonElement jsonelement : jsonarray) {
							ITextComponent itextcomponent1 = this.deserialize(jsonelement, jsonelement.getClass(),
									p_deserialize_3_);
	...
	private void updateLine(NBTTagCompound compound, String key) {
		...
		if (!"null".equals(s) && !StringUtils.isNullOrEmpty(s)) {
			if (s.charAt(0) == '"' && s.charAt(s.length() - 1) == '"'
				...
				try {
					itextcomponent = (ITextComponent) JsonUtils.gsonDeserialize(GSON_INSTANCE, s, ITextComponent.class,
							true);
