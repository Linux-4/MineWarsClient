...
public class CombatTracker {
	...
	public void calculateFallSuffix() {
		...
		if (this.fighter.isOnLadder()) {
			Block block = this.fighter.world.getBlockState(
	...
	public void trackDamage(DamageSource damageSrc, float healthIn, float damageAmount) {
		...
		CombatEntry combatentry = new CombatEntry(damageSrc, this.fighter.ticksExisted, healthIn, damageAmount,
				this.fallSuffix, this.fighter.fallDistance);
				...
				if (combatentry.getDamageSrc() != DamageSource.fall
					...
					if (itextcomponent2 != null
						...
						if (!itemstack1.func_190926_b() && itemstack1.hasDisplayName()) {
							itextcomponent = new TextComponentTranslation("death.fell.assist.item", new Object[] {
									this.fighter.getDisplayName(), itextcomponent2, itemstack1.getTextComponent() });
						} else {
							itextcomponent = new TextComponentTranslation("death.fell.assist",
						...
						if (!itemstack.func_190926_b() && itemstack.hasDisplayName()) {
							itextcomponent = new TextComponentTranslation("death.fell.finish.item", new Object[] {
									this.fighter.getDisplayName(), itextcomponent1, itemstack.getTextComponent() });
						} else {
							itextcomponent = new TextComponentTranslation("death.fell.finish",
									new Object[] { this.fighter.getDisplayName(), itextcomponent1 });
						}
					} else {
						itextcomponent = new TextComponentTranslation("death.fell.killer",
	...
	public EntityLivingBase getBestAttacker() {
		...
		for (CombatEntry combatentry : this.combatEntries) {
			if (combatentry.getDamageSrc().getEntity() instanceof EntityPlayer
					&& (entityplayer == null || combatentry.getDamage() > f1)) {
