...
public class NettyPacketDecoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext p_decode_1_, ByteBuf p_decode_2_, List<Object> p_decode_3_)
		...
		if (p_decode_2_.readableBytes() != 0) {
			...
			Packet<?> packet = ((EnumConnectionState) p_decode_1_.channel().attr(NetworkManager.PROTOCOL_ATTRIBUTE_KEY)
			...
			if (packet == null) {
				...
				if (packetbuffer.readableBytes() > 0) {
					throw new IOException("Packet "
							+ ((EnumConnectionState) p_decode_1_.channel().attr(NetworkManager.PROTOCOL_ATTRIBUTE_KEY)
									.get()).getId()
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(RECEIVED_PACKET_MARKER, " IN: [{}:{}] {}",
								p_decode_1_.channel().attr(NetworkManager.PROTOCOL_ATTRIBUTE_KEY).get(),
