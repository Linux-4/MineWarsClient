...
public class PotionUtils {
	...
	public static ItemStack appendEffects(ItemStack itemIn, Collection<PotionEffect> effects) {
		if (effects.isEmpty()) {
			...
			NBTTagCompound nbttagcompound = (NBTTagCompound) MoreObjects.firstNonNull(itemIn.getTagCompound(),
					new NBTTagCompound());
			...
			for (PotionEffect potioneffect : list) {
				...
				if (!map.isEmpty()) {
					for (Entry<IAttribute, AttributeModifier> entry : map.entrySet()) {
						...
						list1.add(new Tuple(((IAttribute) entry.getKey()).getAttributeUnlocalizedName(),
								attributemodifier1));
					}
				}
