...
public final class ItemStack {
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand,
		...
		EnumActionResult enumactionresult = this.getItem().onItemUse(playerIn, worldIn, pos, hand, side, hitX, hitY,
				hitZ);
	...
	/**
	 * Calls the delegated method to the Item to damage the incoming Entity, and if
	 * necessary, triggers a stats increase.
	 */
	...
	public List<String> getTooltip(@Nullable EntityPlayer playerIn, ITooltipFlag advanced) {
				...
				for (Entry<String, AttributeModifier> entry : multimap.entries()) {
					...
					if (playerIn != null) {
						if (attributemodifier.getID() == Item.ATTACK_DAMAGE_MODIFIER) {
							...
							d0 = d0 + (double) EnchantmentHelper.getModifierForCreature(this,
		...
		if (advanced.func_194127_a()) {
			if (this.isItemDamaged()) {
				list.add(I18n.translateToLocalFormatted("item.durability", this.getMaxDamage() - this.getItemDamage(),
						this.getMaxDamage()));
			}

			list.add(TextFormatting.DARK_GRAY
					+ ((ResourceLocation) Item.REGISTRY.getNameForObject(this.item)).toString());

			if (this.hasTagCompound()) {
				list.add(TextFormatting.DARK_GRAY
						+ I18n.translateToLocalFormatted("item.nbt_tags", this.getTagCompound().getKeySet().size()));
			}
		}
			...
			for (int i = 0; i < nbttaglist.tagCount(); ++i) {
				...
				if (attributemodifier != null
						&& (!nbttagcompound.hasKey("Slot", 8)
								|| nbttagcompound.getString("Slot").equals(equipmentSlot.getName()))
