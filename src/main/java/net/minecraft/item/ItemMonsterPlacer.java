...
public class ItemMonsterPlacer extends Item {
	...
	public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos,
		...
		if (playerIn.isRemote) {
			...
			Entity entity = spawnCreature(playerIn, func_190908_h(itemstack), (double) blockpos.getX() + 0.5D,
		...
		if (itemStackIn.isRemote) {
			...
			if (raytraceresult != null && raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK) {
				...
				if (!(itemStackIn.getBlockState(blockpos).getBlock() instanceof BlockLiquid)) {
					return new ActionResult<ItemStack>(EnumActionResult.PASS, itemstack);
				} else if (itemStackIn.isBlockModifiable(worldIn, blockpos)
						&& worldIn.canPlayerEdit(blockpos, raytraceresult.sideHit, itemstack)) {
					Entity entity = spawnCreature(itemStackIn, func_190908_h(itemstack),
		...
		if (entityID != null && EntityList.ENTITY_EGGS.containsKey(entityID)) {
			...
			for (int i = 0; i < 1; ++i) {
				...
				if (entity instanceof EntityLiving) {
					...
					entity.setLocationAndAngles(x, y, z, MathHelper.wrapDegrees(worldIn.rand.nextFloat() * 360.0F),
							0.0F);
