	...
	public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos,
		...
		if (iblockstate.getBlock() != Blocks.OBSIDIAN && iblockstate.getBlock() != Blocks.BEDROCK) {
			...
			if (!stack.canPlayerEdit(blockpos, hand, itemstack)) {
				...
				boolean flag = !playerIn.isAirBlock(blockpos)
						&& !playerIn.getBlockState(blockpos).getBlock().isReplaceable(playerIn, blockpos);
				flag = flag | (!playerIn.isAirBlock(blockpos1)
						&& !playerIn.getBlockState(blockpos1).getBlock().isReplaceable(playerIn, blockpos1));
				...
				if (flag) {
					...
					List<Entity> list = playerIn.getEntitiesWithinAABBExcludingEntity((Entity) null,
					...
					if (!list.isEmpty()) {
						...
						if (!playerIn.isRemote) {
							EntityEnderCrystal entityendercrystal = new EntityEnderCrystal(playerIn,
									(double) ((float) worldIn.getX() + 0.5F), (double) (worldIn.getY() + 1),
									(double) ((float) worldIn.getZ() + 0.5F));
							entityendercrystal.setShowBottom(false);
							playerIn.spawnEntityInWorld(entityendercrystal);

							if (playerIn.provider instanceof WorldProviderEnd) {
								DragonFightManager dragonfightmanager = ((WorldProviderEnd) playerIn.provider)
