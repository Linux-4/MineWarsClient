	...
	public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos,
		...
		if (hand != EnumFacing.DOWN && (iblockstate.getMaterial().isSolid() || flag)
			...
			if (stack.canPlayerEdit(worldIn, hand, itemstack)
					&& Blocks.STANDING_SIGN.canPlaceBlockAt(playerIn, worldIn)) {
				if (playerIn.isRemote) {
					...
					if (hand == EnumFacing.UP) {
						...
						playerIn.setBlockState(worldIn, Blocks.STANDING_SIGN.getDefaultState()
								.withProperty(BlockStandingSign.ROTATION, Integer.valueOf(i)), 11);
					} else {
						playerIn.setBlockState(worldIn,
								Blocks.WALL_SIGN.getDefaultState().withProperty(BlockWallSign.FACING, hand), 11);
					}
