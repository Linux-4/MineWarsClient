...
public class ItemBoat extends Item {
	...
	public ActionResult<ItemStack> onItemRightClick(World itemStackIn, EntityPlayer worldIn, EnumHand playerIn) {
		...
		if (raytraceresult == null) {
			...
			boolean flag = false;
			List<Entity> list = itemStackIn.getEntitiesWithinAABBExcludingEntity(worldIn, worldIn.getEntityBoundingBox()
			...
			if (flag) {
				...
				EntityBoat entityboat = new EntityBoat(itemStackIn, raytraceresult.hitVec.xCoord,
				...
				if (!itemStackIn.getCollisionBoxes(entityboat, entityboat.getEntityBoundingBox().expandXyz(-0.1D))
						.isEmpty()) {
