...
public class RecipeRepairItem implements IRecipe {
		...
		for (int i = 0; i < inv.getSizeInventory(); ++i) {
			...
			if (!itemstack.func_190926_b()) {
				...
				if (list.size() > 1) {
					...
					if (itemstack.getItem() != itemstack1.getItem() || itemstack1.func_190916_E() != 1
							|| itemstack.func_190916_E() != 1 || !itemstack1.getItem().isDamageable()) {
						return false;
					}
				}
			}
	...
	public ItemStack getCraftingResult(InventoryCrafting inv) {
		...
		if (list.size() == 2) {
			...
			if (itemstack2.getItem() == itemstack3.getItem() && itemstack2.func_190916_E() == 1
					&& itemstack3.func_190916_E() == 1 && itemstack2.getItem().isDamageable()) {
