	...
	public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos,
		...
		if (hand == EnumFacing.DOWN) {
			...
			if (!stack.canPlayerEdit(blockpos, hand, itemstack)) {
				...
				boolean flag1 = !playerIn.isAirBlock(blockpos)
						&& !playerIn.getBlockState(blockpos).getBlock().isReplaceable(playerIn, blockpos);
				flag1 = flag1 | (!playerIn.isAirBlock(blockpos1)
				...
				if (flag1) {
					...
					if (!list.isEmpty()) {
						...
						if (!playerIn.isRemote) {
							...
							EntityArmorStand entityarmorstand = new EntityArmorStand(playerIn, d0 + 0.5D, d1,
									d2 + 0.5D);
							float f = (float) MathHelper
									.floor((MathHelper.wrapDegrees(stack.rotationYaw - 180.0F) + 22.5F) / 45.0F)
									* 45.0F;
							...
							playerIn.playSound((EntityPlayer) null, entityarmorstand.posX, entityarmorstand.posY,
