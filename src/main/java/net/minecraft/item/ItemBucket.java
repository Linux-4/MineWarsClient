	...
	public ActionResult<ItemStack> onItemRightClick(World itemStackIn, EntityPlayer worldIn, EnumHand playerIn) {
		...
		if (raytraceresult == null) {
			...
			if (!itemStackIn.isBlockModifiable(worldIn, blockpos)) {
				...
				if (!worldIn.canPlayerEdit(blockpos.offset(raytraceresult.sideHit), raytraceresult.sideHit,
						itemstack)) {
					return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
				} else {
					...
					if (material == Material.WATER
						...
						return new ActionResult<ItemStack>(EnumActionResult.SUCCESS,
								this.fillBucket(itemstack, worldIn, Items.WATER_BUCKET));
						...
						worldIn.addStat(StatList.getObjectUseStats(this));
						return new ActionResult<ItemStack>(EnumActionResult.SUCCESS,
								this.fillBucket(itemstack, worldIn, Items.LAVA_BUCKET));
					} else {
						return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
					}
				}
				...
				if (!worldIn.canPlayerEdit(blockpos1, raytraceresult.sideHit, itemstack)) {
					...
					worldIn.addStat(StatList.getObjectUseStats(this));
					return !worldIn.capabilities.isCreativeMode
