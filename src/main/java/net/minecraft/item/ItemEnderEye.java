...
public class ItemEnderEye extends Item {
	...
	public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos,
		...
		if (stack.canPlayerEdit(worldIn.offset(hand), hand, itemstack)
			...
			if (playerIn.isRemote) {
				...
				playerIn.setBlockState(worldIn,
						iblockstate.withProperty(BlockEndPortalFrame.EYE, Boolean.valueOf(true)), 2);
				...
				playerIn.playSound((EntityPlayer) null, worldIn, SoundEvents.field_193781_bp, SoundCategory.BLOCKS,
						1.0F, 1.0F);
				BlockPattern.PatternHelper blockpattern$patternhelper = BlockEndPortalFrame.getOrCreatePortalShape()
						.match(playerIn, worldIn);
	...
	public ActionResult<ItemStack> onItemRightClick(World itemStackIn, EntityPlayer worldIn, EnumHand playerIn) {
		...
		if (raytraceresult != null && raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK
				&& itemStackIn.getBlockState(raytraceresult.getBlockPos()).getBlock() == Blocks.END_PORTAL_FRAME) {
			return new ActionResult<ItemStack>(EnumActionResult.PASS, itemstack);
		} else {
			worldIn.setActiveHand(playerIn);

			if (!itemStackIn.isRemote) {
				BlockPos blockpos = ((WorldServer) itemStackIn).getChunkProvider().getStrongholdGen(itemStackIn,
						"Stronghold", new BlockPos(worldIn), false);

				if (blockpos != null) {
					EntityEnderEye entityendereye = new EntityEnderEye(itemStackIn, worldIn.posX,
							worldIn.posY + (double) (worldIn.height / 2.0F), worldIn.posZ);
					...
					itemStackIn.playSound((EntityPlayer) null, worldIn.posX, worldIn.posY, worldIn.posZ,
