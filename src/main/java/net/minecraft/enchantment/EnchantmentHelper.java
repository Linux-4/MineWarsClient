...
public class EnchantmentHelper {
	...
	/**
	 * Checks if the player has any armor enchanted with the frost walker
	 * enchantment.
	 * 
	...
	public static ItemStack getEnchantedItem(Enchantment p_92099_0_, EntityLivingBase p_92099_1_) {
		...
		if (list.isEmpty()) {
			...
			return list1.isEmpty() ? ItemStack.field_190927_a
					: (ItemStack) list1.get(p_92099_1_.getRNG().nextInt(list1.size()));
		}
	}

	/**
	 * Returns the enchantability of itemstack, using a separate calculation for
	 * each enchantNum (0, 1 or 2), cutting to the max enchantability power of the
	 * table, which is locked to a max of 15.
	 */
	 ...
	 */
	public static ItemStack addRandomEnchantment(Random random, ItemStack p_77504_1_, int p_77504_2_,
	...
	public static List<EnchantmentData> buildEnchantmentList(Random randomIn, ItemStack itemStackIn, int p_77513_2_,
		...
		if (i <= 0) {
			...
			p_77513_2_ = MathHelper.clamp(Math.round((float) p_77513_2_ + (float) p_77513_2_ * f), 1,
					Integer.MAX_VALUE);
