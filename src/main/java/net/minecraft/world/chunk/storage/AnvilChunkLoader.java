...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO {
	...
	public boolean func_191063_a(int p_191063_1_, int p_191063_2_) {
		...
		NBTTagCompound nbttagcompound = this.chunksToRemove.get(chunkpos);
		return nbttagcompound != null ? true
				: RegionFileCache.func_191064_f(this.chunkSaveLocation, p_191063_1_, p_191063_2_);
	}
	...
	protected Chunk checkedReadChunkFromNBT(World worldIn, int x, int z, NBTTagCompound compound) {
		if (!compound.hasKey("Level", 10)) {
			...
			if (!nbttagcompound.hasKey("Sections", 9)) {
				LOGGER.error("Chunk file at {},{} is missing block data, skipping", Integer.valueOf(x),
						Integer.valueOf(z));
				return null;
			} else {
				...
				if (!chunk.isAtLocation(x, z)) {
					LOGGER.error(
							...
							Integer.valueOf(x), Integer.valueOf(z), Integer.valueOf(x), Integer.valueOf(z),
							Integer.valueOf(chunk.xPosition), Integer.valueOf(chunk.zPosition));
	...
	private void writeChunkData(ChunkPos pos, NBTTagCompound compound) throws IOException {
		DataOutputStream dataoutputstream = RegionFileCache.getChunkOutputStream(this.chunkSaveLocation, pos.chunkXPos,
				pos.chunkZPos);
				...
				if (compound.hasKey("Level", 10)) {
					...
					if (nbttagcompound.hasKey("Entities", 9)) {
						...
						for (int i = 0; i < nbttaglist.tagCount(); ++i) {
							nbttaglist.set(i,
									fixer.process(FixTypes.ENTITY, (NBTTagCompound) nbttaglist.get(i), versionIn));
						}
					}
					...
					if (nbttagcompound.hasKey("TileEntities", 9)) {
						...
						for (int j = 0; j < nbttaglist1.tagCount(); ++j) {
							nbttaglist1.set(j, fixer.process(FixTypes.BLOCK_ENTITY, (NBTTagCompound) nbttaglist1.get(j),
									versionIn));
						}
					}
				}
		...
		if (entity == null) {
			...
			if (attemptSpawn && !worldIn.spawnEntityInWorld(entity)) {
				...
				if (compound.hasKey("Passengers", 9)) {
					...
					for (int i = 0; i < nbttaglist.tagCount(); ++i) {
						Entity entity1 = readWorldEntityPos(nbttaglist.getCompoundTagAt(i), worldIn, x, y, z,
								attemptSpawn);
