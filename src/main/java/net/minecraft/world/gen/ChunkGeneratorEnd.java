...
public class ChunkGeneratorEnd implements IChunkGenerator {
	...
	public void populate(int x, int z) {
		...
		if (i > 4096L) {
			...
			if (f < -20.0F && this.rand.nextInt(14) == 0) {
				this.endIslands.generate(this.worldObj, this.rand,
						blockpos.add(this.rand.nextInt(16) + 8, 55 + this.rand.nextInt(16), this.rand.nextInt(16) + 8));

				if (this.rand.nextInt(4) == 0) {
					this.endIslands.generate(this.worldObj, this.rand, blockpos.add(this.rand.nextInt(16) + 8,
			...
			if (this.getIslandHeightValue(x, z, 1, 1) > 40.0F) {
				...
				for (int k = 0; k < j; ++k) {
					...
					if (j1 > 0) {
						...
						if (this.worldObj.isAirBlock(blockpos.add(l, k1 + 1, i1)) && this.worldObj
								.getBlockState(blockpos.add(l, k1, i1)).getBlock() == Blocks.END_STONE) {
	...
	public BlockPos getStrongholdGen(World worldIn, String structureName, BlockPos position, boolean p_180513_4_) {
		return "EndCity".equals(structureName) && this.endCityGen != null
				? this.endCityGen.getClosestStrongholdPos(worldIn, position, p_180513_4_)
				: null;
	}
