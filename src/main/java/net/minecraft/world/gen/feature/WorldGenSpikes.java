...
public class WorldGenSpikes extends WorldGenerator {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		if (this.spike == null) {
			...
			for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(
				...
				if (blockpos$mutableblockpos.distanceSq((double) position.getX(),
						(double) blockpos$mutableblockpos.getY(), (double) position.getZ()) <= (double) (i * i + 1)
						&& blockpos$mutableblockpos.getY() < this.spike.getHeight()) {
					this.setBlockAndNotifyAdequately(worldIn, blockpos$mutableblockpos,
			...
			entityendercrystal.setLocationAndAngles((double) ((float) position.getX() + 0.5F),
					(double) (this.spike.getHeight() + 1), (double) ((float) position.getZ() + 0.5F),
	...
	/**
	 * Sets the value that will be used in a call to entitycrystal.setBeamTarget. At
	 * the moment, WorldGenSpikes.setBeamTarget is only ever called with a value of
	 * (0, 128, 0)
	 */
	...
	public static class EndSpike {
		...
		public EndSpike(int p_i47020_1_, int p_i47020_2_, int p_i47020_3_, int p_i47020_4_, boolean p_i47020_5_) {
			...
			this.topBoundingBox = new AxisAlignedBB((double) (p_i47020_1_ - p_i47020_3_), 0.0D,
					(double) (p_i47020_2_ - p_i47020_3_), (double) (p_i47020_1_ + p_i47020_3_), 256.0D,
