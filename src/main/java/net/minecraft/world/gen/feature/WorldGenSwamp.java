...
public class WorldGenSwamp extends WorldGenAbstractTree {
	private static final IBlockState TRUNK = Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT,
			...
			if (!flag) {
				...
				if ((block1 == Blocks.GRASS || block1 == Blocks.DIRT) && position.getY() < 256 - i - 1) {
					...
					for (int l1 = 0; l1 < i; ++l1) {
						...
						if (iblockstate1.getMaterial() == Material.AIR || iblockstate1.getMaterial() == Material.LEAVES
	...
	private void addVine(World worldIn, BlockPos pos, PropertyBool prop) {
		...
		for (BlockPos blockpos = pos.down(); worldIn.getBlockState(blockpos).getMaterial() == Material.AIR
				&& i > 0; --i) {
