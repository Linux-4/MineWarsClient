...
public class MapData extends WorldSavedData {
	...
	public void updateVisiblePlayers(EntityPlayer player, ItemStack mapStack) {
		...
		for (int i = 0; i < this.playersArrayList.size(); ++i) {
			...
			if (!mapdata$mapinfo1.entityplayerObj.isDead
				...
				if (!mapStack.isOnItemFrame() && mapdata$mapinfo1.entityplayerObj.dimension == this.dimension
					...
					this.func_191095_a(MapDecoration.Type.PLAYER, mapdata$mapinfo1.entityplayerObj.world,
		...
		if (mapStack.isOnItemFrame() && this.trackingPosition) {
			...
			this.func_191095_a(MapDecoration.Type.FRAME, player.world, "frame-" + entityitemframe.getEntityId(),
		...
		if (mapStack.hasTagCompound() && mapStack.getTagCompound().hasKey("Decorations", 9)) {
			...
			for (int j = 0; j < nbttaglist.tagCount(); ++j) {
				...
				if (!this.mapDecorations.containsKey(nbttagcompound.getString("id"))) {
					this.func_191095_a(MapDecoration.Type.func_191159_a(nbttagcompound.getByte("type")), player.world,
	...
	public static void func_191094_a(ItemStack p_191094_0_, BlockPos p_191094_1_, String p_191094_2_,
	...
	private void func_191095_a(MapDecoration.Type p_191095_1_, World p_191095_2_, String p_191095_3_,
			double p_191095_4_, double p_191095_6_, double p_191095_8_) {
		int i = 1 << this.scale;
	...
	public class MapInfo {
		...
		public Packet<?> getPacket(ItemStack stack) {
			if (this.isDirty) {
				this.isDirty = false;
				return new SPacketMaps(stack.getMetadata(), MapData.this.scale, MapData.this.trackingPosition,
				...
				return this.tick++ % 5 == 0
						? new SPacketMaps(stack.getMetadata(), MapData.this.scale, MapData.this.trackingPosition,
