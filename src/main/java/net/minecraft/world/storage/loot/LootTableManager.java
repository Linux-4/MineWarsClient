	...
	class Loader extends CacheLoader<ResourceLocation, LootTable> {
		...
		private LootTable loadLootTable(ResourceLocation resource) {
			if (LootTableManager.this.baseFolder == null) {
				...
				File file1 = new File(new File(LootTableManager.this.baseFolder, resource.getResourceDomain()),
				...
				if (file1.exists()) {
					if (file1.isFile()) {
						...
						try {
							return (LootTable) JsonUtils.gsonDeserialize(LootTableManager.GSON_INSTANCE, s,
									LootTable.class);
		...
		private LootTable loadBuiltinLootTable(ResourceLocation resource) {
			...
			if (url != null) {
				...
				try {
					...
					LootTableManager.LOGGER.error("Couldn't load loot table {} from {}", resource, url,
							jsonparseexception);
					return LootTable.EMPTY_LOOT_TABLE;
				}
			} else {
