...
public class PhaseSittingFlaming extends PhaseSittingBase {
	...
	public void doClientRenderEffects() {
		...
		if (this.flameTicks % 2 == 0 && this.flameTicks < 10) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				for (int j = 0; j < 6; ++j) {
					this.dragon.world.spawnParticle(EnumParticleTypes.DRAGON_BREATH, d3, d4, d5,
							-vec3d.xCoord * 0.07999999821186066D * (double) j, -vec3d.yCoord * 0.6000000238418579D,
							-vec3d.zCoord * 0.07999999821186066D * (double) j);
				}
	...
	public void doLocalUpdate() {
		...
		if (this.flameTicks >= 200) {
			...
			Vec3d vec3d = (new Vec3d(this.dragon.dragonPartHead.posX - this.dragon.posX, 0.0D,
			...
			BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos(MathHelper.floor(d0),
					MathHelper.floor(d2), MathHelper.floor(d1));
