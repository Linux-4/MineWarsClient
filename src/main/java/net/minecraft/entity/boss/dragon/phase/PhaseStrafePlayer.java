		...
		if (this.attackTarget == null) {
			...
			if (this.attackTarget.getDistanceSqToEntity(this.dragon) < 4096.0D) {
				if (this.dragon.canEntityBeSeen(this.attackTarget)) {
					++this.fireballCharge;
					Vec3d vec3d1 = (new Vec3d(this.attackTarget.posX - this.dragon.posX, 0.0D,
							this.attackTarget.posZ - this.dragon.posZ)).normalize();
					Vec3d vec3d = (new Vec3d((double) MathHelper.sin(this.dragon.rotationYaw * 0.017453292F), 0.0D,
					...
					if (this.fireballCharge >= 5 && f >= 0.0F && f < 10.0F) {
						...
						double d7 = this.dragon.dragonPartHead.posY
								+ (double) (this.dragon.dragonPartHead.height / 2.0F) + 0.5D;
						...
						EntityDragonFireball entitydragonfireball = new EntityDragonFireball(this.dragon.world,
								this.dragon, d9, d10, d11);
