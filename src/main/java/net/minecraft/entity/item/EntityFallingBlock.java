			...
			if (!this.world.isRemote) {
				...
				if (flag && d0 > 1.0D) {
					...
					if (raytraceresult != null
							&& this.world.getBlockState(raytraceresult.getBlockPos()).getMaterial() == Material.WATER) {
				...
				if (!this.onGround && !flag1) {
					...
					if (!flag1 && BlockFalling.canFallThrough(this.world
							.getBlockState(new BlockPos(this.posX, this.posY - 0.009999999776482582D, this.posZ)))) {
						this.onGround = false;
						return;
					}
					...
					if (iblockstate.getBlock() != Blocks.PISTON_EXTENSION) {
						...
						if (!this.canSetAsBlock) {
							if (this.world.func_190527_a(block, blockpos1, true, EnumFacing.UP, (Entity) null)
									&& (flag1
								...
								if (block instanceof BlockFalling) {
									((BlockFalling) block).onEndFalling(this.world, blockpos1, this.fallTile,
											iblockstate);
								}
