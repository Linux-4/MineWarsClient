...
public class EntityFireworkRocket extends Entity {
	private static final DataParameter<ItemStack> FIREWORK_ITEM = EntityDataManager
			.<ItemStack>createKey(EntityFireworkRocket.class, DataSerializers.OPTIONAL_ITEM_STACK);
	private static final DataParameter<Integer> field_191512_b = EntityDataManager
		...
		if (this.func_191511_j()) {
			...
			if (this.field_191513_e != null) {
				if (this.field_191513_e.isElytraFlying()) {
					...
					double d0 = 1.5D;
					double d1 = 0.1D;
					this.field_191513_e.motionX += vec3d.xCoord * 0.1D
							+ (vec3d.xCoord * 1.5D - this.field_191513_e.motionX) * 0.5D;
					this.field_191513_e.motionY += vec3d.yCoord * 0.1D
							+ (vec3d.yCoord * 1.5D - this.field_191513_e.motionY) * 0.5D;
					this.field_191513_e.motionZ += vec3d.zCoord * 0.1D
							+ (vec3d.zCoord * 1.5D - this.field_191513_e.motionZ) * 0.5D;
				}
	...
	private void func_191510_k() {
		...
		if (f > 0.0F) {
			if (this.field_191513_e != null) {
				this.field_191513_e.attackEntityFrom(DamageSource.field_191552_t,
						(float) (5 + nbttaglist.tagCount() * 2));
			}
			...
			for (EntityLivingBase entitylivingbase : this.world.getEntitiesWithinAABB(EntityLivingBase.class,
				...
				if (entitylivingbase != this.field_191513_e && this.getDistanceSqToEntity(entitylivingbase) <= 25.0D) {
					...
					for (int i = 0; i < 2; ++i) {
						RayTraceResult raytraceresult = this.world.rayTraceBlocks(vec3d,
					...
					if (flag) {
						float f1 = f * (float) Math
								.sqrt((5.0D - (double) this.getDistanceToEntity(entitylivingbase)) / 5.0D);
