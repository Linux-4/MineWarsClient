...
public abstract class AbstractHorse extends EntityAnimal implements IInventoryChangedListener, IJumpingMount {
	...
	protected boolean canMate() {
		return !this.isBeingRidden() && !this.isRiding() && this.isTame() && !this.isChild()
	...
	protected void func_190681_a(EntityAgeable p_190681_1_, AbstractHorse p_190681_2_) {
		...
		double d1 = this.getEntityAttribute(JUMP_STRENGTH).getBaseValue()
				+ p_190681_1_.getEntityAttribute(JUMP_STRENGTH).getBaseValue() + this.getModifiedJumpStrength();
	...
	public void updatePassenger(Entity passenger) {
		...
		if (this.prevRearingAmount > 0.0F) {
			...
			passenger.setPosition(this.posX + (double) (f1 * f3),
