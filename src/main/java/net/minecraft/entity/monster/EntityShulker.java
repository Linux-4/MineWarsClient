...
public class EntityShulker extends EntityGolem implements IMob {
	...
	protected static final DataParameter<Byte> PEEK_TICK = EntityDataManager.<Byte>createKey(EntityShulker.class,
			DataSerializers.BYTE);
	protected static final DataParameter<Byte> field_190770_bw = EntityDataManager.<Byte>createKey(EntityShulker.class,
			DataSerializers.BYTE);
	...
	protected boolean tryTeleportToNewPosition() {
		if (!this.isAIDisabled() && this.isEntityAlive()) {
			...
			for (int i = 0; i < 5; ++i) {
				...
				if (blockpos1.getY() > 0 && this.world.isAirBlock(blockpos1) && this.world.func_191503_g(this)
			...
			if (EntityShulker.this.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
				...
				if (d0 < 400.0D) {
					if (this.attackTime <= 0) {
						...
						EntityShulkerBullet entityshulkerbullet = new EntityShulkerBullet(EntityShulker.this.world,
								EntityShulker.this, entitylivingbase,
								EntityShulker.this.getAttachmentFacing().getAxis());
						EntityShulker.this.world.spawnEntityInWorld(entityshulkerbullet);
						EntityShulker.this.playSound(SoundEvents.ENTITY_SHULKER_SHOOT, 2.0F,
								(EntityShulker.this.rand.nextFloat() - EntityShulker.this.rand.nextFloat()) * 0.2F
										+ 1.0F);
					}
				} else {
