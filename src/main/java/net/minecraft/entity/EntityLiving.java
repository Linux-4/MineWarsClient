...
public abstract class EntityLiving extends EntityLivingBase {
	...
	private final NonNullList<ItemStack> inventoryHands = NonNullList.<ItemStack>func_191197_a(2,
	...
	private final NonNullList<ItemStack> inventoryArmor = NonNullList.<ItemStack>func_191197_a(4,
			ItemStack.field_190927_a);
	...
	protected int getExperiencePoints(EntityPlayer player) {
		if (this.experienceValue > 0) {
			...
			for (int j = 0; j < this.inventoryArmor.size(); ++j) {
				if (!((ItemStack) this.inventoryArmor.get(j)).func_190926_b()
						&& this.inventoryArmorDropChances[j] <= 1.0F) {
					i += 1 + this.rand.nextInt(3);
				}
			}

			for (int k = 0; k < this.inventoryHands.size(); ++k) {
				if (!((ItemStack) this.inventoryHands.get(k)).func_190926_b()
						&& this.inventoryHandsDropChances[k] <= 1.0F) {
					i += 1 + this.rand.nextInt(3);
				}
			}
	...
	public void onLivingUpdate() {
		...
		if (!this.world.isRemote && this.canPickUpLoot() && !this.dead
	...
	protected void despawnEntity() {
		...
		if (this.persistenceRequired) {
			this.entityAge = 0;
				...
				&& (object = Reflector.call(Reflector.ForgeEventFactory_canEntityDespawn, this)) != object1) {
			if (object == object2) {
				this.entityAge = 0;
			} else {
				this.setDead();
	...
	protected void dropEquipment(boolean wasRecentlyHit, int lootingModifier) {
		for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
			...
			if (!itemstack.func_190926_b() && !EnchantmentHelper.func_190939_c(itemstack) && (wasRecentlyHit || flag)
	...
	protected void setEnchantmentBasedOnDifficulty(DifficultyInstance difficulty) {
		...
		if (!this.getHeldItemMainhand().func_190926_b() && this.rand.nextFloat() < 0.25F * f) {
			this.setItemStackToSlot(EntityEquipmentSlot.MAINHAND, EnchantmentHelper.addRandomEnchantment(this.rand,
					this.getHeldItemMainhand(), (int) (5.0F + f * (float) this.rand.nextInt(18)), false));
		}
		...
		for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
			if (entityequipmentslot.getSlotType() == EntityEquipmentSlot.Type.ARMOR) {
				...
				if (!itemstack.func_190926_b() && this.rand.nextFloat() < 0.5F * f) {
					this.setItemStackToSlot(entityequipmentslot, EnchantmentHelper.addRandomEnchantment(this.rand,
							itemstack, (int) (5.0F + f * (float) this.rand.nextInt(18)), false));
				}
			}
		}
	}
	...
	/**
	 * Return the persistenceRequired field (whether this entity is allowed to
	 * naturally despawn)
	 */
		...
		if (this.isLeashed) {
			...
			if (!this.world.isRemote && sendPacket && this.world instanceof WorldServer) {
				((WorldServer) this.world).getEntityTracker().sendToAllTrackingEntity(this,
						new SPacketEntityAttach(this, (Entity) null));
			}
		}
	}
	...
	public void setLeashedToEntity(Entity entityIn, boolean sendAttachNotification) {
		...
		if (!this.world.isRemote && sendAttachNotification && this.world instanceof WorldServer) {
			((WorldServer) this.world).getEntityTracker().sendToAllTrackingEntity(this,
					new SPacketEntityAttach(this, this.leashedToEntity));
		}
		...
		if (this.isLeashed && this.leashNBTTag != null) {
			if (this.leashNBTTag.hasUniqueId("UUID")) {
				...
				for (EntityLivingBase entitylivingbase : this.world.getEntitiesWithinAABB(EntityLivingBase.class,
						this.getEntityBoundingBox().expandXyz(10.0D))) {
	...
	public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
		...
		if (!itemStackIn.func_190926_b() && !isItemStackInSlot(entityequipmentslot, itemStackIn)
				&& entityequipmentslot != EntityEquipmentSlot.HEAD) {
			return false;
		} else {
