...
public class EntityAIMate extends EntityAIBase {
	...
	private EntityAnimal getNearbyMate() {
		List<EntityAnimal> list = this.theWorld.<EntityAnimal>getEntitiesWithinAABB(this.field_190857_e,
	...
	private void spawnBaby() {
		...
		if (entityageable != null) {
			...
			if (entityplayermp != null) {
				...
				CriteriaTriggers.field_192134_n.func_192168_a(entityplayermp, this.theAnimal, this.targetMate,
						entityageable);
			}
			...
			for (int i = 0; i < 7; ++i) {
				...
				this.theWorld.spawnParticle(EnumParticleTypes.HEART, this.theAnimal.posX + d3, this.theAnimal.posY + d4,
						this.theAnimal.posZ + d5, d0, d1, d2);
			}
