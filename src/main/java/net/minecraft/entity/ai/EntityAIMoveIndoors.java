...
public class EntityAIMoveIndoors extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if ((!this.entityObj.world.isDaytime()
				|| this.entityObj.world.isRaining() && !this.entityObj.world.getBiome(blockpos).canRain())
				&& this.entityObj.world.provider.func_191066_m()) {
	...
	public void startExecuting() {
		...
		if (this.entityObj.getDistanceSq(blockpos) > 256.0D) {
			Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockTowards(this.entityObj, 14, 3,
