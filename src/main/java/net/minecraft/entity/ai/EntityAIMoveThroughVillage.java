...
public class EntityAIMoveThroughVillage extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (this.isNocturnal && this.theEntity.world.isDaytime()) {
			...
			Village village = this.theEntity.world.getVillageCollection()
				...
				if (this.doorInfo == null) {
					...
					if (this.entityPathNavigate != null) {
						...
						Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockTowards(this.theEntity, 10, 7,
								new Vec3d((double) this.doorInfo.getDoorBlockPos().getX(),
						...
						if (vec3d == null) {
							...
							pathnavigateground.setBreakDoors(false);
							this.entityPathNavigate = this.theEntity.getNavigator().getPathToXYZ(vec3d.xCoord,
	...
	private VillageDoorInfo findNearestDoor(Village villageIn) {
		...
		for (VillageDoorInfo villagedoorinfo1 : villageIn.getVillageDoorInfoList()) {
			int j = villagedoorinfo1.getDistanceSquared(MathHelper.floor(this.theEntity.posX),
					MathHelper.floor(this.theEntity.posY), MathHelper.floor(this.theEntity.posZ));
