...
public class EntityTracker {
	...
	public void addEntityToTracker(Entity entityIn, int trackingRange, final int updateFrequency,
		...
		try {
			...
			EntityTrackerEntry entitytrackerentry = new EntityTrackerEntry(entityIn, trackingRange,
					this.maxTrackingDistanceThreshold, updateFrequency, sendVelocityUpdates);
	...
	/**
	 * Send packets to player for every tracked entity in this chunk that is either
	 * leashed to something or someone, or has passengers
	 */
	public void sendLeashedEntitiesInChunk(EntityPlayerMP player, Chunk chunkIn) {
		...
		for (EntityTrackerEntry entitytrackerentry : this.trackedEntities) {
			...
			if (entity != player && entity.chunkCoordX == chunkIn.xPosition
					&& entity.chunkCoordZ == chunkIn.zPosition) {
				entitytrackerentry.updatePlayerEntity(player);
		...
		if (!list.isEmpty()) {
			for (Entity entity1 : list) {
				player.connection
						.sendPacket(new SPacketEntityAttach(entity1, ((EntityLiving) entity1).getLeashedToEntity()));
			}
		}
