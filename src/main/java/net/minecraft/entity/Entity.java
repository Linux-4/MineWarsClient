...
public abstract class Entity implements ICommandSender {
	...
	protected static final DataParameter<Byte> FLAGS = EntityDataManager.<Byte>createKey(Entity.class,
	...
	private static final DataParameter<Boolean> CUSTOM_NAME_VISIBLE = EntityDataManager.<Boolean>createKey(Entity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> SILENT = EntityDataManager.<Boolean>createKey(Entity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> NO_GRAVITY = EntityDataManager.<Boolean>createKey(Entity.class,
			DataSerializers.BOOLEAN);
		...
		if (width != this.width || height != this.height) {
			...
			if (this.width < f) {
				...
				this.setEntityBoundingBox(new AxisAlignedBB(this.posX - d0, this.posY, this.posZ - d0, this.posX + d0,
						this.posY + (double) this.height, this.posZ + d0));
				return;
			}
	...
	public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
		if (this.noClip) {
			...
			if ((x == MoverType.SELF || x == MoverType.PLAYER) && this.onGround && this.isSneaking()
				...
				for (double d5 = 0.05D; p_70091_2_ != 0.0D && this.world
						.getCollisionBoxes(this,
								this.getEntityBoundingBox().offset(p_70091_2_, (double) (-this.stepHeight), 0.0D))
				...
				for (; p_70091_6_ != 0.0D && this.world
						.getCollisionBoxes(this,
								this.getEntityBoundingBox().offset(0.0D, (double) (-this.stepHeight), p_70091_6_))
				...
				for (; p_70091_2_ != 0.0D && p_70091_6_ != 0.0D
						&& this.world.getCollisionBoxes(this,
								this.getEntityBoundingBox().offset(p_70091_2_, (double) (-this.stepHeight), p_70091_6_))
								.isEmpty(); d4 = p_70091_6_) {
					if (p_70091_2_ < 0.05D && p_70091_2_ >= -0.05D) {
						p_70091_2_ = 0.0D;
					} else if (p_70091_2_ > 0.0D) {
			...
			List<AxisAlignedBB> list1 = this.world.getCollisionBoxes(this,
					this.getEntityBoundingBox().addCoord(p_70091_2_, p_70091_4_, p_70091_6_));
			AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
			...
			if (p_70091_4_ != 0.0D) {
				...
				for (int l = list1.size(); k < l; ++k) {
					p_70091_4_ = ((AxisAlignedBB) list1.get(k)).calculateYOffset(this.getEntityBoundingBox(),
							p_70091_4_);
				}
			...
			if (p_70091_2_ != 0.0D) {
				...
				for (int l5 = list1.size(); j5 < l5; ++j5) {
					p_70091_2_ = ((AxisAlignedBB) list1.get(j5)).calculateXOffset(this.getEntityBoundingBox(),
							p_70091_2_);
				}
			...
			if (p_70091_6_ != 0.0D) {
				...
				for (int i6 = list1.size(); k5 < i6; ++k5) {
					p_70091_6_ = ((AxisAlignedBB) list1.get(k5)).calculateZOffset(this.getEntityBoundingBox(),
							p_70091_6_);
				}
			...
			if (this.stepHeight > 0.0F && flag && (d2 != p_70091_2_ || d4 != p_70091_6_)) {
				...
				for (int k4 = list.size(); j4 < k4; ++j4) {
					p_70091_4_ = ((AxisAlignedBB) list.get(j4)).calculateYOffset(this.getEntityBoundingBox(),
							p_70091_4_);
				}
			...
			if (this.canTriggerWalking() && (!this.onGround || !this.isSneaking() || !(this instanceof EntityPlayer))
				...
				} else if (this.distanceWalkedOnStepModified > this.field_191959_ay && this.func_191957_ae()
						&& iblockstate.getMaterial() == Material.AIR) {
	...
	public void playSound(SoundEvent soundIn, float volume, float pitch) {
		if (!this.isSilent()) {
			this.world.playSound((EntityPlayer) null, this.posX, this.posY, this.posZ, soundIn, this.getSoundCategory(),
					volume, pitch);
		}
	}
	...
	public boolean func_191953_am() {
		return this.world.handleMaterialAcceleration(
				this.getEntityBoundingBox().expand(0.0D, -20.0D, 0.0D).contract(0.001D), Material.WATER, this);
	}
	...
	protected void resetHeight() {
		...
		float f1 = MathHelper.sqrt(entity.motionX * entity.motionX * 0.20000000298023224D
				+ entity.motionY * entity.motionY + entity.motionZ * entity.motionZ * 0.20000000298023224D) * f;
		...
		for (int i = 0; (float) i < 1.0F + this.width * 20.0F; ++i) {
			...
			this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double) f3, (double) (f2 + 1.0F),
					this.posZ + (double) f4, this.motionX, this.motionY - (double) (this.rand.nextFloat() * 0.2F),
					this.motionZ);
		}
		...
		for (int j = 0; (float) j < 1.0F + this.width * 20.0F; ++j) {
			...
			this.world.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (double) f5, (double) (f2 + 1.0F),
					this.posZ + (double) f6, this.motionX, this.motionY, this.motionZ);
		}
	}
	...
	public int getBrightnessForRender() {
		BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos(MathHelper.floor(this.posX), 0,
	...
	public EntityItem entityDropItem(ItemStack stack, float offsetY) {
		if (stack.func_190926_b()) {
			...
			EntityItem entityitem = new EntityItem(this.world, this.posX, this.posY + (double) offsetY, this.posZ,
					stack);
	...
	public void setPortal(BlockPos pos) {
		if (this.timeUntilPortal > 0) {
			...
			if (!this.world.isRemote && !pos.equals(this.lastPortalPos)) {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = Blocks.PORTAL.createPatternHelper(this.world,
						this.lastPortalPos);
	...
	public String toString() {
		return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]", this.getClass().getSimpleName(),
				this.getName(), this.entityId, this.world == null ? "~NULL~" : this.world.getWorldInfo().getWorldName(),
