...
public abstract class EntityLivingBase extends Entity {
	...
	private final NonNullList<ItemStack> handInventory = NonNullList.<ItemStack>func_191197_a(2,
	...
	protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos) {
		...
		if (!this.world.isRemote && this.fallDistance > 3.0F && onGroundIn) {
			...
			if (state.getMaterial() != Material.AIR) {
				...
				int i = (int) (150.0D * d0);
				((WorldServer) this.world).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ,
						i, 0.0D, 0.0D, 0.0D, 0.15000000596046448D, Block.getStateId(state));
			}
		}
		...
		if (this.isEntityInvulnerable(source)) {
			...
			if (this.getHealth() <= 0.0F) {
				...
				if (entity1 instanceof EntityPlayerMP) {
					CriteriaTriggers.field_192127_g.func_192220_a((EntityPlayerMP) entity1, this, source, f, amount,
							flag);
				}
	...
	public void renderBrokenItemStack(ItemStack stack) {
		...
		for (int i = 0; i < 5; ++i) {
			...
			this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec3d1.xCoord, vec3d1.yCoord, vec3d1.zCoord,
					vec3d.xCoord, vec3d.yCoord + 0.05D, vec3d.zCoord, Item.getIdFromItem(stack.getItem()));
		}
	}
	 ...
	 * 
	 * @param strength Magnitude of the knockback vector, and also the Y-velocity to
	 *                 add if the entity is on the ground.
			...
			if (enumfacing1 != null) {
				...
				for (int[] aint : aint1) {
					...
					if (!this.world.collidesWithAnyBlock(axisalignedbb1)) {
						...
						if (this.world.getBlockState(blockpos).isFullyOpaque()
							...
							&& this.world.getBlockState(new BlockPos(d11, this.posY + 1.0D, d12)).isFullyOpaque()) {
						d1 = d11;
						d13 = this.posY + 2.0D;
						d14 = d12;
					}
	...
	public void onItemPickup(Entity entityIn, int quantity) {
		if (!entityIn.isDead && !this.world.isRemote) {
			...
			if (entityIn instanceof EntityItem || entityIn instanceof EntityArrow || entityIn instanceof EntityXPOrb) {
				entitytracker.sendToAllTrackingEntity(entityIn,
			...
			if (stack.getItemUseAction() == EnumAction.EAT) {
				for (int i = 0; i < eatingParticleCount; ++i) {
					...
					if (stack.getHasSubtypes()) {
						this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec3d1.xCoord, vec3d1.yCoord,
