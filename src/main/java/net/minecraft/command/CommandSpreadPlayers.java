...
public class CommandSpreadPlayers extends CommandBase {
		...
		if (args.length < 6) {
			...
			if (list.isEmpty()) {
				...
				sender.addChatMessage(
						new TextComponentTranslation("commands.spreadplayers.spreading." + (flag ? "teams" : "players"),
	...
	private void spread(ICommandSender sender, List<Entity> p_110669_2_, CommandSpreadPlayers.Position pos,
		...
		CommandSpreadPlayers.Position[] acommandspreadplayers$position = this.createInitialPositions(random,
				respectTeams ? this.getNumberOfTeams(p_110669_2_) : p_110669_2_.size(), d0, d1, d2, d3);
		int i = this.spreadPositions(pos, spreadDistance, worldIn, random, d0, d1, d2, d3,
				acommandspreadplayers$position, respectTeams);
		...
		if (acommandspreadplayers$position.length > 1) {
			sender.addChatMessage(
					new TextComponentTranslation("commands.spreadplayers.info." + (respectTeams ? "teams" : "players"),
	...
	private int spreadPositions(CommandSpreadPlayers.Position p_110668_1_, double p_110668_2_, World worldIn,
			Random random, double minX, double minZ, double maxX, double maxZ,
			CommandSpreadPlayers.Position[] p_110668_14_, boolean respectTeams) throws CommandException {
		boolean flag = true;
		...
		if (i >= 10000) {
			throw new CommandException("commands.spreadplayers.failure." + (respectTeams ? "teams" : "players"),
	...
	private double setPlayerPositions(List<Entity> p_110671_1_, World worldIn,
		...
		for (int j = 0; j < p_110671_1_.size(); ++j) {
			...
			entity.setPositionAndUpdate((double) ((float) MathHelper.floor(commandspreadplayers$position.x) + 0.5F),
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		return args.length >= 1 && args.length <= 2 ? getTabCompletionCoordinateXZ(args, 0, pos)
