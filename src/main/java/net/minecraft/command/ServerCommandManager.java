...
public class ServerCommandManager extends CommandHandler implements ICommandListener {
	...
	public void notifyListener(ICommandSender sender, ICommand command, int flags, String translationKey,
		...
		ITextComponent itextcomponent = new TextComponentTranslation("chat.type.admin",
		...
		if (flag) {
			for (EntityPlayer entityplayer : minecraftserver.getPlayerList().getPlayerList()) {
				if (entityplayer != sender
						&& minecraftserver.getPlayerList().canSendCommands(entityplayer.getGameProfile())
						&& command.checkPermission(this.server, sender)) {
