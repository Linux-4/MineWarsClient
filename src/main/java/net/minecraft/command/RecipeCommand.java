...
public class RecipeCommand extends CommandBase {
				...
				for (EntityPlayerMP entityplayermp : func_193513_a(server, sender, args[1])) {
					if ("*".equals(args[2])) {
						if (flag) {
							...
							notifyCommandListener(sender, this, "commands.recipe.give.success.all",
									new Object[] { entityplayermp.getName() });
						} else {
							entityplayermp.func_192022_b(this.func_192556_d());
							notifyCommandListener(sender, this, "commands.recipe.take.success.all",
									new Object[] { entityplayermp.getName() });
						}
					} else {
						...
						if (flag == entityplayermp.func_192037_E().func_193830_f(irecipe)) {
							...
							throw new CommandException(s, new Object[] { entityplayermp.getName(),
									irecipe.getRecipeOutput().getDisplayName() });
						}

						if (flag) {
							entityplayermp.func_192021_a(list);
							notifyCommandListener(sender, this, "commands.recipe.give.success.one", new Object[] {
									entityplayermp.getName(), irecipe.getRecipeOutput().getDisplayName() });
						} else {
							entityplayermp.func_192022_b(list);
							notifyCommandListener(sender, this, "commands.recipe.take.success.one", new Object[] {
									irecipe.getRecipeOutput().getDisplayName(), entityplayermp.getName() });
						}
					}
				}
			}
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		if (args.length == 1) {
			...
			return args.length == 3 ? getListOfStringsMatchingLastWord(args, CraftingManager.field_193380_a.getKeys())
