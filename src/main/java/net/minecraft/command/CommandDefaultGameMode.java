...
public class CommandDefaultGameMode extends CommandGameMode {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length <= 0) {
			...
			notifyCommandListener(sender, this, "commands.defaultgamemode.success",
					new Object[] { new TextComponentTranslation("gameMode." + gametype.getName(), new Object[0]) });
		}
	}

	/**
	 * Set the default game type for the server. Also propogate the changes to all
	 * players if the server is set to force game mode
	 */
