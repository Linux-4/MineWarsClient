...
public class CommandBanIp extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length >= 1 && args[0].length() > 1) {
			...
			if (matcher.matches()) {
				this.banIp(server, sender, args[0],
				...
				this.banIp(server, sender, entityplayermp.getPlayerIP(),
						itextcomponent == null ? null : itextcomponent.getUnformattedText());
			}
		} else {
	...
	protected void banIp(MinecraftServer server, ICommandSender sender, String ipAddress, @Nullable String banReason) {
		UserListIPBansEntry userlistipbansentry = new UserListIPBansEntry(ipAddress, (Date) null, sender.getName(),
				(Date) null, banReason);
		...
		for (EntityPlayerMP entityplayermp : list) {
			entityplayermp.connection
					.func_194028_b(new TextComponentTranslation("multiplayer.disconnect.ip_banned", new Object[0]));
		...
		if (list.isEmpty()) {
			...
			notifyCommandListener(sender, this, "commands.banip.success.players",
