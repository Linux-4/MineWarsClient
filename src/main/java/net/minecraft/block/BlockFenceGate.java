...
public class BlockFenceGate extends BlockHorizontal {
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
		...
		if (((Boolean) state.getValue(IN_WALL)).booleanValue()) {
			return ((EnumFacing) state.getValue(FACING)).getAxis() == EnumFacing.Axis.X ? AABB_COLLIDE_XAXIS_INWALL
					: AABB_COLLIDE_ZAXIS_INWALL;
		} else {
			return ((EnumFacing) state.getValue(FACING)).getAxis() == EnumFacing.Axis.X ? AABB_COLLIDE_XAXIS
					: AABB_COLLIDE_ZAXIS;
		}
	}
	...
	public AxisAlignedBB getCollisionBoundingBox(IBlockState blockState, IBlockAccess worldIn, BlockPos pos) {
		if (((Boolean) blockState.getValue(OPEN)).booleanValue()) {
			...
			return ((EnumFacing) blockState.getValue(FACING)).getAxis() == EnumFacing.Axis.Z
	...
	public BlockFaceShape func_193383_a(IBlockAccess p_193383_1_, IBlockState p_193383_2_, BlockPos p_193383_3_,
		...
		if (p_193383_4_ != EnumFacing.UP && p_193383_4_ != EnumFacing.DOWN) {
			return ((EnumFacing) p_193383_2_.getValue(FACING)).getAxis() == p_193383_4_.rotateY().getAxis()
