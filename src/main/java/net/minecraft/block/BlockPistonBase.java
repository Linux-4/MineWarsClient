...
public class BlockPistonBase extends BlockDirectional {
	...
	protected static final AxisAlignedBB PISTON_BASE_SOUTH_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 1.0D,
			0.75D);
	protected static final AxisAlignedBB PISTON_BASE_NORTH_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.25D, 1.0D, 1.0D,
			1.0D);
	...
	public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ,
		...
		return this.getDefaultState().withProperty(FACING, EnumFacing.func_190914_a(pos, placer)).withProperty(EXTENDED,
	 ...
	 * Server, this may perform additional changes to the world, like pistons
	 * replacing the block with an extended base. On the client, the update may
	 * involve replacing tile entities, playing sounds, or performing other visual
	 * actions to reflect the server side changes.
	 */
	public boolean eventReceived(IBlockState state, World worldIn, BlockPos pos, int id, int param) {
		...
		if (id == 0) {
			...
			if (this.isSticky) {
				...
				if (!flag1 && iblockstate.getMaterial() != Material.AIR
		...
		if (!blockpistonstructurehelper.canMove()) {
			...
			if (extending) {
				...
				worldIn.setTileEntity(blockpos2,
						BlockPistonMoving.createTilePiston(iblockstate3, direction, true, true));
			}
	...
	public BlockFaceShape func_193383_a(IBlockAccess p_193383_1_, IBlockState p_193383_2_, BlockPos p_193383_3_,
		...
		return p_193383_2_.getValue(FACING) != p_193383_4_.getOpposite()
				&& ((Boolean) p_193383_2_.getValue(EXTENDED)).booleanValue() ? BlockFaceShape.UNDEFINED
