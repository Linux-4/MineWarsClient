...
public class BlockDoublePlant extends BlockBush implements IGrowable {
	...
	public BlockDoublePlant() {
		...
		this.setDefaultState(this.blockState.getBaseState()
				...
				.withProperty(HALF, BlockDoublePlant.EnumBlockHalf.LOWER).withProperty(FACING, EnumFacing.NORTH));
	...
	public boolean isReplaceable(IBlockAccess worldIn, BlockPos pos) {
		...
		if (iblockstate.getBlock() != this) {
			...
			BlockDoublePlant.EnumPlantType blockdoubleplant$enumplanttype = (BlockDoublePlant.EnumPlantType) iblockstate
					.getActualState(worldIn, pos).getValue(VARIANT);
			...
			if (worldIn.getBlockState(pos.down()).getBlock() == this) {
				if (player.capabilities.isCreativeMode) {
					...
					if (blockdoubleplant$enumplanttype != BlockDoublePlant.EnumPlantType.FERN
						...
						worldIn.setBlockToAir(pos.down());
					} else if (!player.getHeldItemMainhand().func_190926_b()
