...
public class GMatrix implements java.io.Serializable, Cloneable {
	 ...
	 */
	public final void copySubMatrix(int rowSource, int colSource, int numRow, int numCol, int rowDest, int colDest,
			GMatrix target) {
		int i, j;
		...
		for (si = 0; si < sLength; si++) {
			...
			if (nr > 1) {
				...
				for (i = 0; i < nr; i++) {
					...
					if (debug)
						System.out.println(
								"mag = " + mag + " matrix.dot = " + m.values[i + si][si] * m.values[i + si][si]);
				}
	...
	private static void update_v_split(int topr, int bottomr, GMatrix v, double[] cosr, double[] sinr, GMatrix t,
			GMatrix m) {
		int j;
		double vtemp;
	...
	private static void update_u_split(int topr, int bottomr, GMatrix u, double[] cosl, double[] sinl, GMatrix t,
			GMatrix m) {
		int j;
		double utemp;
