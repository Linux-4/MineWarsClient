...
public class SmartLeaves {
	...
	public static boolean isSameLeaves(IBlockState p_isSameLeaves_0_, IBlockState p_isSameLeaves_1_) {
		if (p_isSameLeaves_0_ == p_isSameLeaves_1_) {
			...
			if (block != block1) {
				...
				return ((BlockPlanks.EnumType) p_isSameLeaves_0_.getValue(BlockOldLeaf.VARIANT))
						.equals(p_isSameLeaves_1_.getValue(BlockOldLeaf.VARIANT));
			} else {
				return block instanceof BlockNewLeaf
						? ((BlockPlanks.EnumType) p_isSameLeaves_0_.getValue(BlockNewLeaf.VARIANT))
								.equals(p_isSameLeaves_1_.getValue(BlockNewLeaf.VARIANT))
						: false;
			}
		}
	}
	...
	private static List getGeneralQuadsSafe(IBakedModel p_getGeneralQuadsSafe_0_) {
		return p_getGeneralQuadsSafe_0_ == null ? null
				: p_getGeneralQuadsSafe_0_.getQuads((IBlockState) null, (EnumFacing) null, 0L);
	}
					...
					if (ibakedmodel != null && ibakedmodel != modelmanager.getMissingModel()) {
						...
						if (list.size() == 0) {
							...
							for (Object bakedquad : list) {
								List list1 = ibakedmodel.getQuads((IBlockState) null, ((BakedQuad) bakedquad).getFace(),
										0L);
	...
	private static IBakedModel getModelDoubleFace(IBakedModel p_getModelDoubleFace_0_) {
		if (p_getModelDoubleFace_0_ == null) {
			...
			Config.warn("SmartLeaves: Model is not cube, general quads: "
					+ p_getModelDoubleFace_0_.getQuads((IBlockState) null, (EnumFacing) null, 0L).size() + ", model: "
					+ p_getModelDoubleFace_0_);
