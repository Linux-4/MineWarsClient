...
public class TextureAnimations {
	...
	public static TextureAnimation[] getTextureAnimations(IResourcePack p_getTextureAnimations_0_) {
		String[] astring = ResUtils.collectFiles(p_getTextureAnimations_0_, "mcpatcher/anim/", ".properties",
				(String[]) null);
		...
		if (s != null && s1 != null) {
			if (i >= 0 && j >= 0 && k >= 0 && l >= 0) {
				...
				if (abyte == null) {
					...
					if (i1 != k1) {
						Config.warn("TextureAnimation: Source texture has invalid number of frames: " + s + ", frames: "
								+ (float) i1 / (float) (k * l));
						return null;
					} else {
						...
						try {
							...
							if (inputstream == null) {
								...
								if (i + k <= bufferedimage.getWidth() && j + l <= bufferedimage.getHeight()) {
									TextureAnimation textureanimation = new TextureAnimation(s, abyte, s1,
											resourcelocation, i, j, k, l, p_makeTextureAnimation_0_, 1);
									return textureanimation;
								} else {
									Config.warn(
											"TextureAnimation: Animation coordinates are outside the target texture: "
													+ s1);
									return null;
								}
							}
