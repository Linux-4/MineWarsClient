...
public class SpriteDependencies {
	public static TextureAtlasSprite resolveDependencies(List<TextureAtlasSprite> p_resolveDependencies_0_,
			int p_resolveDependencies_1_, TextureMap p_resolveDependencies_2_) {
		TextureAtlasSprite textureatlassprite;

		for (textureatlassprite = p_resolveDependencies_0_.get(p_resolveDependencies_1_); resolveOne(
				p_resolveDependencies_0_, p_resolveDependencies_1_, textureatlassprite,
				p_resolveDependencies_2_); textureatlassprite = p_resolveDependencies_0_
						.get(p_resolveDependencies_1_)) {
			;
		}
	...
	private static boolean resolveOne(List<TextureAtlasSprite> p_resolveOne_0_, int p_resolveOne_1_,
			TextureAtlasSprite p_resolveOne_2_, TextureMap p_resolveOne_3_) {
		int i = 0;
		...
		for (ResourceLocation resourcelocation : p_resolveOne_2_.getDependencies()) {
			...
			if (textureatlassprite == null) {
				...
				if (textureatlassprite.isDependencyParent) {
					String s = "circular dependency: " + p_resolveOne_2_.getIconName() + " -> "
