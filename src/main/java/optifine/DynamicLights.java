...
public class DynamicLights {
	...
	private static final DataParameter<ItemStack> PARAMETER_ITEM_STACK = new DataParameter<ItemStack>(6,
	...
	public static void entityRemoved(Entity p_entityRemoved_0_, RenderGlobal p_entityRemoved_1_) {
		synchronized (mapDynamicLights) {
			DynamicLight dynamiclight = mapDynamicLights
					.remove(net.minecraft.util.IntegerCache.getInteger(p_entityRemoved_0_.getEntityId()));
		...
		if (p_getLightLevel_0_ == null) {
			...
			if (item == Items.LAVA_BUCKET) {
				...
				if (item == Items.GLOWSTONE_DUST) {
					...
					return item == Items.NETHER_STAR ? Blocks.BEACON.getLightValue(Blocks.BEACON.getDefaultState()) / 2
							: 0;
				}
			} else {
				return 10;
