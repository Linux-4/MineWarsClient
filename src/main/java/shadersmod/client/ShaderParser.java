...
public class ShaderParser {
	...
	public static Pattern PATTERN_CONST_FLOAT = Pattern
			.compile("\\s*const\\s+float\\s+(\\w+)\\s*=\\s*([-+.\\w]+)\\s*;.*");
			...
			if (matcher1.matches()) {
				...
				if (matcher2.matches()) {
					...
					if (matcher3.matches()) {
						...
						return matcher4.matches() ? new ShaderLine(5, matcher4.group(1), matcher4.group(2), line)
								: null;
					}
				}
			}
